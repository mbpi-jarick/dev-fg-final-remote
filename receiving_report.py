...
# --- PDF & Printing Imports (Unchanged) ---
import fitz  # PyMuPDF
...
# --- PyQt6 Imports ---
from PyQt6.QtCore import Qt, QDate, pyqtSignal, QSizeF, QDateTime, QSize
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QTabWidget, QFormLayout, QLineEdit, QDateEdit,
                             ...
                             QDialog, QDialogButtonBox, QGroupBox, QMenu, QGridLayout, QSplitter, QInputDialog, QFrame)
...
# --- UPDATED UI CONSTANTS (Harmonized with Outgoing Form Style) ---
...
# Rename constants to match new theme structure, but keep originals for QTAWESOME color definitions
PRIMARY_ACCENT_COLOR = COLOR_PRIMARY  # Used for primary actions/icons
DESTRUCTIVE_COLOR = COLOR_DANGER  # Used for delete actions/icons
SECONDARY_ACCENT_COLOR = COLOR_SECONDARY  # Used for secondary actions/icons
...
class FloatLineEdit(QLineEdit):
    def __init__(self, parent=None):
        ...
        # Keep validator as standard, users can only type digits/decimal point/sign easily
        validator = QDoubleValidator(0.0, 9999999999.0, 2)
        validator.setNotation(QDoubleValidator.Notation.StandardNotation)
        self.setValidator(validator)
        self.editingFinished.connect(self._format_text)
        self.setAlignment(Qt.AlignmentFlag.AlignRight)
        self.setText("0.00")  # Initialize with formatted text

    def _format_text(self):
        try:
            # 1. Clean text by removing commas before converting to float
            clean_text = self.text().replace(',', '')
            value = float(clean_text or 0.0)
            # 2. Format with comma separator and two decimal places
            self.setText(f"{value:,.2f}")
        except ValueError:
            self.setText("0.00")

    def value(self) -> float:
        # Clean text before conversion when retrieving the value
        clean_text = self.text().replace(',', '')
...
class AddNewRecordDialog(QDialog):
    def __init__(self, parent, title, label, table_name, engine):
        ...
        self.setStyleSheet(f"QDialog {{ background-color: {INPUT_BACKGROUND_COLOR}; }}")

        # Apply light button style
        ok_btn = button_box.button(QDialogButtonBox.StandardButton.Ok)
        cancel_btn = button_box.button(QDialogButtonBox.StandardButton.Cancel)
        ok_btn.setObjectName("PrimaryButton")
        cancel_btn.setObjectName("DefaultButton")  # Changed to Default for Cancel
        ok_btn.setIcon(fa.icon('fa5s.check', color=COLOR_PRIMARY))
        cancel_btn.setIcon(fa.icon('fa5s.times', color=COLOR_DEFAULT))
...
class AddItemDialog(QDialog):
    ...
    def __init__(self, parent=None, data=None, warehouses=None, engine=None):
        ...
        self.check_lot_btn = QPushButton("Verify Lot")
        ...
        self.check_lot_btn.setIcon(fa.icon('fa5s.search', color=COLOR_SECONDARY))
        self.check_lot_btn.setObjectName("SecondaryButton")
        # -------------------------------------------
        ...
        # --- QTAWESOME ICON & LIGHT BUTTON STYLE ---
        add_location_btn.setIcon(fa.icon('fa5s.cogs', color=COLOR_DEFAULT))
        add_location_btn.setObjectName("DefaultButton")
        # -------------------------------------------
        ...
        self.ok_button.setIcon(fa.icon('fa5s.check', color=COLOR_PRIMARY))
        cancel_btn.setIcon(fa.icon('fa5s.times', color=COLOR_DEFAULT))
        ...
        if data:
            ...
            # When setting text, FloatLineEdit's _format_text will handle the display comma
    ...
    # (Other methods in AddItemDialog remain unchanged as they are logic related)
    ...
    def get_data(self):
        # We retrieve the raw text from FloatLineEdit, which will now contain commas (e.g., "1,234.00")
        # This is fine because when we parse it later, we strip the commas.
...
class ReceivingReportPage(QWidget):
    ...
    def _get_styles(self) -> str:
        return f"""
            ...
            /* Standard Labels (must be white background for consistent theme) */
            QLabel {{
            ...
            /* Instruction Box Style (QFrame with class set) */
            ...
            /* Primary Button (Save/Restore/Update) */
            ...
            /* Secondary Button (Fetch/Edit/Print/Refresh) */
            ...
            /* Default Button (Manage lists, New, Cancel) */
    ...
    def init_ui(self):
        ...
        # Icon
        icon_pixmap = fa.icon('fa5s.truck-loading', color="#3a506b").pixmap(QSize(32, 32))
        icon_label = QLabel()
        icon_label.setPixmap(icon_pixmap)
        header_layout.addWidget(icon_label, alignment=Qt.AlignmentFlag.AlignLeft)

        # Title
        title_label = QLabel("Receiving Report (RRRG) Management", objectName="PageHeader")
        ...
        # Tabs added with Qtawesome icons
        self.tab_widget.addTab(self.view_tab, fa.icon('fa5s.list-alt', color=COLOR_PRIMARY), "All Reports")
        self.tab_widget.addTab(self.entry_tab, fa.icon('fa5s.file-alt', color=COLOR_PRIMARY), "Report Entry")
        self.tab_widget.addTab(self.view_details_tab, fa.icon('fa5s.eye', color=COLOR_SECONDARY), "View Details")
    ...
    def _setup_deleted_tab(self, tab):
        ...
        instruction_label = QLabel(
            "<b>Deleted Reports:</b> Records shown here are soft-deleted. Use the context menu or button to restore them to the active reports list, which re-applies their inventory transactions."
        )
        ...
        self.deleted_refresh_btn.setIcon(fa.icon('fa5s.sync-alt', color=COLOR_DEFAULT))
        self.deleted_refresh_btn.setObjectName("DefaultButton")
        self.deleted_refresh_btn.setMinimumWidth(120)
    ...
    # (Logic methods for deleted tab remain unchanged)
    ...
    def _setup_view_tab(self, tab):
        ...
        instruction_layout = QHBoxLayout(instruction_frame)
        instruction_label = QLabel(
            "<b>All Reports:</b> Browse, search, update, or delete existing Receiving Reports (RRRG). Double-click a row to load for editing."
        )
        instruction_label.setWordWrap(True)
        instruction_layout.addWidget(instruction_label)
        layout.addWidget(instruction_frame)
        ...
        self.refresh_btn.setIcon(fa.icon('fa5s.sync-alt', color=COLOR_DEFAULT))
        self.update_btn.setIcon(fa.icon('fa5s.edit', color=COLOR_PRIMARY))
        self.delete_btn.setIcon(fa.icon('fa5s.trash-alt', color=COLOR_DANGER))
        ...
        # --- QTAWESOME ICON & LIGHT BUTTON STYLE ---
        self.prev_btn.setIcon(fa.icon('fa5s.chevron-left', color=COLOR_PRIMARY))
        self.next_btn.setIcon(fa.icon('fa5s.chevron-right', color=COLOR_PRIMARY))
        self.prev_btn.setObjectName("PrimaryButton")
        self.next_btn.setObjectName("PrimaryButton")
        # -------------------------------------------

        self.page_label = QLabel("Page 1 of 1")
        self.page_label.setStyleSheet("background-color: transparent;")  # Override QLabel style
    ...
    def _setup_view_details_tab(self, tab):
        ...
        instruction_layout = QHBoxLayout(instruction_frame)
        instruction_label = QLabel(
            "<b>View Details:</b> Detailed read-only view of the selected Receiving Report (RRRG). Use the 'All Reports' tab to select a record."
        )
        instruction_label.setWordWrap(True)
        instruction_layout.addWidget(instruction_label)
        layout.addWidget(instruction_frame)
    ...
    def _setup_entry_tab(self, tab):
        ...
        instruction_layout = QHBoxLayout(instruction_frame)
        instruction_label = QLabel(
            "<b>RRG Form:</b> Fill out the Receiving Report (RRRG) form. Ensure all item details, quantities, statuses, and locations are correctly entered. The RRRG No will be auto-generated upon saving a new report."
        )
        instruction_label.setWordWrap(True)
        instruction_layout.addWidget(instruction_label)
        layout.addWidget(instruction_frame)
        ...
        # --- QTAWESOME ICON & LIGHT BUTTON STYLE ---
        add_row_btn.setIcon(fa.icon('fa5s.plus', color=COLOR_SUCCESS))
        edit_row_btn.setIcon(fa.icon('fa5s.pen', color=COLOR_SECONDARY))
        remove_row_btn.setIcon(fa.icon('fa5s.minus', color=COLOR_DANGER))
        add_row_btn.setObjectName("PrimaryButton")
        edit_row_btn.setObjectName("SecondaryButton")
        remove_row_btn.setObjectName("delete_btn")
        # -------------------------------------------
        ...
        self.clear_btn.setIcon(fa.icon('fa5s.file-alt', color=COLOR_DEFAULT))
        self.cancel_update_btn.setIcon(fa.icon('fa5s.times', color=COLOR_DANGER))
        self.print_btn.setIcon(fa.icon('fa5s.print', color=COLOR_SECONDARY))
    ...
    def _create_combo_with_add_button(self, combo, on_add_clicked):
        ...
        # --- QTAWESOME ICON & LIGHT BUTTON STYLE ---
        add_btn.setIcon(fa.icon('fa5s.cogs', color=COLOR_DEFAULT))
        add_btn.setObjectName("DefaultButton")
        # -------------------------------------------
    ...
    # (All remaining methods related to logic, database, and PDF are unchanged)
    ...
    def _show_selected_record_in_view_tab(self):
        ...
        try:
            ...
            for item_data in items:
                # Pass DB data, which should be float, for formatting
                self._add_item_row_to_table(dict(item_data), table=self.view_items_table)
        except Exception as e:
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            for item_data in items:
                # Pass DB data, which should be float, for formatting
                self._add_item_row_to_table(dict(item_data), table=self.items_table)
    ...
    def _add_item_row_to_table(self, data, table):
        ...
        # --- QTY FORMATTING MODIFICATION START ---
        try:
        ...
        # --- QTY FORMATTING MODIFICATION END ---
    ...
    def _show_add_item_dialog(self):
        ...
        if dialog.exec():
            # dialog.get_data() returns the string from FloatLineEdit, which is already comma formatted.
            self._add_item_row_to_table(dialog.get_data(), table=self.items_table)

    def _show_edit_item_dialog(self):
        row = self.items_table.currentRow()
        if row < 0: return
        # The quantity text retrieved here already has commas, which FloatLineEdit handles fine.
        ...
        if dialog.exec():
            ...
            # FloatLineEdit ensures new_data['quantity_kg'] is comma formatted.
    ...
    def _show_records_context_menu(self, position):
        ...
        # --- QTAWESOME ICONS ---
        view_action = menu.addAction(fa.icon('fa5s.eye', color=COLOR_SECONDARY), "View Details")
        ...
        menu.addSeparator()
        print_action = menu.addAction(fa.icon('fa5s.print', color=COLOR_SUCCESS), "Print Preview")
        # -----------------------
    ...
    def _save_record(self):
        ...
        for row in range(self.items_table.rowCount()):
            try:
                # Retrieve the string value (may contain commas)
                qty_string = self.items_table.item(row, 2).text()

                # Convert to float, stripping commas for database consistency
                quantity_kg = float(qty_string.replace(',', ''))

                item_data = {
                    "material_code": self.items_table.item(row, 0).text().strip(),
                    "lot_no": self.items_table.item(row, 1).text().strip(),
                    "quantity_kg": quantity_kg,  # Use the clean float value
                    "status": self.items_table.item(row, 3).text().strip(),
                    "location": self.items_table.item(row, 4).text().strip()
                }

                if not all([item_data["material_code"], item_data["lot_no"], item_data["status"],
                            item_data["location"]]) or quantity_kg <= 0:
    ...
    def _delete_record(self):
        ...
        if not ok: return
        # Hardcoded admin password check (as per original logic)
        if password != "Itadmin":
    ...
    # (PDF and remaining helper methods unchanged)
    def _print_from_entry_form(self):
        ...
        items_data = []
        for row in range(self.items_table.rowCount()):
            # Quantity text here already contains commas
            items_data.append({
    ...
    def _print_selected_record(self):
        ...
        try:
            ...
            # items_db contains raw float values from the database
    ...
    def _generate_report_pdf(self, primary_data, items_data):
        ...
        for item in items_data:
            ...
            # --- QTY FORMATTING MODIFICATION START ---
            try:
                # Need to handle case where quantity_kg might be a string (from entry form) containing commas
                qty_float = float(str(qty_val).replace(',', ''))
                qty_str = f"{qty_float:,.2f}"  # Format with comma
            except (ValueError, TypeError):
                qty_str = "0.00"
            # --- QTY FORMATTING MODIFICATION END ---
