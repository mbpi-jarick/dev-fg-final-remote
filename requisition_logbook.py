        ...
        for row_idx, record in enumerate(data):
            for col_idx, key in enumerate(keys_in_order):
                ...
                if isinstance(value, (Decimal, float)):
                    text_val = format_float_with_commas(value)  # Use helper here
                elif isinstance(value, (date, datetime)):
                    text_val = value.strftime('%Y-%m-%d %H:%M')
                elif isinstance(value, str) and ('date' in key or 'on' in key) and len(value) > 10 and value[
                    10] == ' ':
                    try:
    ...
    def _show_selected_record_in_view_tab(self):
        ...
        try:
            ...
            for key, value in record.items():
                ...
                if isinstance(value, datetime):
                ...
                elif isinstance(value, str) and len(value) > 10 and value[10] == ' ':
                    try:
                        dt_obj = datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
                    ...
                    display_value = format_float_with_commas(value)  # Use helper here
                else:
                    display_value = str(value or 'N/A').upper()
