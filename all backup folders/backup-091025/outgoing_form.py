# outgoing_form.py
# UPDATE - Integrated a non-intrusive notification bar for all actions and added a refresh button to the deleted records tab for consistency.
...
# REFINED: Removed dashboard for a cleaner UI, reordered tabs to prioritize the record list, and updated button styles to match the main application theme.
...
class OutgoingFormPage(QWidget):
    ...
    def show_notification(self, message: str, level: str = 'info', duration_ms: int = 5000):
        """Displays a temporary notification message at the bottom of the page."""
        self.notification_timer.stop()
        level_styles = {
            'info': "background-color: #3498db; color: white;",  # Blue
            'success': "background-color: #2ecc71; color: white;", # Green
            'warning': "background-color: #f39c12; color: white;", # Orange
            'error': "background-color: #e74c3c; color: white;"   # Red
        }
    ...
    def _setup_deleted_tab(self, tab):
        ...
        self.restore_btn.setObjectName("PrimaryButton") # Maps to blue
    ...
    def _setup_view_tab(self, tab):
        ...
        self.update_btn.setObjectName("SecondaryButton") # Maps to green
        self.delete_btn = QPushButton("Delete Selected")
        self.delete_btn.setObjectName("delete_btn") # Maps to red
    ...
    def _setup_entry_tab(self, tab):
        ...
        self.fetch_details_btn.setObjectName("SecondaryButton") # Green
        ...
        self.add_item_btn.setObjectName("PrimaryButton") # Blue
        self.edit_item_btn = QPushButton("Edit Selected");
        self.edit_item_btn.setObjectName("SecondaryButton") # Green
        self.remove_item_btn = QPushButton("Remove Selected");
        self.remove_item_btn.setObjectName("remove_item_btn") # Red
        ...
        self.save_btn.setObjectName("PrimaryButton") # Blue
        self.clear_btn = QPushButton("New"); # Default gray
        self.cancel_update_btn = QPushButton("Cancel Update");
        self.cancel_update_btn.setObjectName("delete_btn") # Red
        button_layout.addStretch();
    ...
    def _on_tab_changed(self, index):
        ...
        if tab_text == "All Outgoing Forms":
            self._load_all_records()
    ...
    def _refresh_all_data_views(self):
        """Reloads data for all relevant views (main table, deleted table)."""
        self._load_all_records()
        self._load_deleted_records()
    ...
    def _on_record_selection_changed(self):
        ...
        if not is_selected and self.tab_widget.currentIndex() == self.tab_widget.indexOf(self.view_details_tab):
            self.tab_widget.setCurrentIndex(0) # Change to the first tab (All Records)
    ...
    def _load_all_records(self):
        ...
        try:
            ...
            notification_message = f"Loaded {len(results)} of {self.total_records} records."
            if self.sender() == self.refresh_btn:
                notification_message = f"Table refreshed. Loaded {len(results)} records."
            self.show_notification(notification_message, 'info', duration_ms=3000)
    ...
    def _load_deleted_records(self):
        ...
        try:
            ...
            notification_message = f"Loaded {len(results)} deleted records."
            if self.sender() == self.deleted_refresh_btn:
                notification_message = f"Deleted records refreshed. Found {len(results)}."
            self.show_notification(notification_message, 'info', duration_ms=3000)
