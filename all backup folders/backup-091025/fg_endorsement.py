# fg_endorsement.py
# UPDATE - Added a refresh button to data tables for manual reloading and feedback.
# UPDATE - Integrated a simple, non-intrusive notification bar at the bottom to provide feedback for every action, reducing pop-ups for a cleaner UI.
...
# REFINED - Removed dashboard for a cleaner, more focused interface and updated button colors to match the application-wide theme.
...
class FGEndorsementPage(QWidget):
    ...
    def _setup_entry_tab(self, tab):
        ...
        self.preview_btn.setObjectName("SecondaryButton")  # Maps to green
        self.save_btn = QPushButton("Save Endorsement");
        self.save_btn.setObjectName("PrimaryButton")  # Maps to blue
        self.clear_btn = QPushButton("New");  # Uses default gray style
        self.cancel_update_btn = QPushButton("Cancel Update");
        self.cancel_update_btn.setObjectName("delete_btn")  # Maps to red
        button_layout.addStretch()
    ...
    def _on_tab_changed(self, index):
        ...
        if tab_text == "All Endorsement Records":
            self._load_all_endorsements()
    ...
    def _load_all_endorsements(self):
        ...
        try:
            ...
            # This notification now serves for initial load, search, and refresh
            notification_message = f"Loaded {len(res)} of {self.total_records} records."
            if self.sender() == self.refresh_btn:
                notification_message = f"Table refreshed. Loaded {len(res)} records."
    ...
    def _load_deleted_endorsements(self):
        ...
        try:
            ...
            notification_message = f"Loaded {len(res)} deleted records."
            if self.sender() == self.deleted_refresh_btn:
                notification_message = f"Deleted records refreshed. Found {len(res)}."
            self.show_notification(notification_message, 'info', duration_ms=3000)
