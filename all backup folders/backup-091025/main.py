...
class AppStyles:
    ...
    MAIN_WINDOW_STYLESHEET = f"""
        ...
        #SideMenu #MenuLabel:first-of-type {{
            border-top: none;
        }}
        ...
        #SideMenu QPushButton {{
            ...
            color: #ecf0f1;
            border: none;
            padding: 9px 9px 9px 20px; /* Adjusted padding for collapsed view */
            text-align: left;
            font-size: 9pt;
            font-weight: normal;
            border-radius: 6px;
...
class ModernMainWindow(QMainWindow):
    # --- NEW: Constants for collapsible menu ---
    ...
    def __init__(self, username, user_role, login_window):
        ...
        # --- NEW: State for collapsible menu ---
    ...
    def init_ui(self):
        ...
        self.side_menu = self.create_side_menu()  # Store reference
    ...
    def create_side_menu(self):
        self.menu_buttons = []  # Initialize list to hold buttons
        ...
        layout.addSpacing(10)  # Add some space after profile
        ...
        # --- Add Buttons to Layout (without labels and with separators) ---
        # Group 1: Management Forms
    ...
    def create_menu_button(self, text, icon, page_index, on_click_func=None):
        ...
        btn.setProperty("fullText", text)  # Store full text for restoring
        ...
        self.menu_buttons.append(btn)  # Add to list for text manipulation
        return btn
    ...
    def toggle_side_menu(self):
        ...
        # If we are about to collapse, hide the text and profile info now.
        ...
        # Unconditionally connect the finished signal to our handler.
    ...
    def restore_menu_texts(self):
        # This slot is called only when expanding
        if self.is_menu_expanded:
    ...
    def create_status_widget(self, icon_name, initial_text, icon_color='#6c757d'):
        """Helper function to create a consistent widget for the status bar."""
        widget = QWidget()
        ...
        # Store labels as attributes of the widget for easy access later
    ...
    def on_menu_animation_finished(self):
        # The animation is complete, now we can safely update the state.
        self.is_menu_expanded = not self.is_menu_expanded

        # If the new state is 'expanded', restore the texts.
        if self.is_menu_expanded:
            self.profile_name_label.setVisible(True)
            self.profile_role_label.setVisible(True)
            for button in self.menu_buttons:
