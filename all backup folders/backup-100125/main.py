...
class AppStyles:
    ...
    # --- MODIFIED: Side Menu/Login Background Gradient ---
    ...
    LOGIN_STYLESHEET = f"""
        #LoginWindow, #FormFrame {{ 
            background: {SIDE_MENU_GRADIENT}; /* MODIFIED: Use dark gradient for login background */
        }}
        QWidget {{ font-family: "Segoe UI"; font-size: 11pt; color: {DARK_TEXT_COLOR}; }}

        #LoginTitle {{ 
            font-size: 20pt; font-weight: bold; 
            color: {PRIMARY_ACCENT_COLOR}; /* Use accent color for title */
        }}

        /* Input Frame needs a separate, lighter background to stand out */
        #InputFrame {{ 
            background-color: {DARK_MENU_SECONDARY}; /* Lighter dark gray for input box frame */
            border: 1px solid #495057; 
            border-radius: 8px; padding: 5px; 
        }}
        #InputFrame:focus-within {{ border: 2px solid {PRIMARY_ACCENT_COLOR}; }}

        /* Input line edit text needs to be light gray on the dark input background */
        QLineEdit {{ 
            border: none; 
            background-color: transparent; 
            padding: 8px; 
            font-size: 11pt;
            color: {DARK_TEXT_COLOR}; /* White text in input field */
        }}

        /* Login icon color set to primary blue */
        #InputFrame QLabel {{ color: {PRIMARY_ACCENT_COLOR}; }}
...
class LoginWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        # Set window icon color to primary accent
        ...
        # MODIFIED: Use the primary accent color for the main login icon (Colored Icon Request)
    ...
    def _create_input_field(self, icon_name, placeholder):
        ...
        l.setSpacing(10)
        # Icon color is determined by the stylesheet's #InputFrame QLabel rule using PRIMARY_ACCENT_COLOR
        pixmap = IconProvider.get_pixmap(icon_name, AppStyles.PRIMARY_ACCENT_COLOR, QSize(20, 20))
...
# --- MODIFIED: ModernMainWindow with dark theme updates ---
class ModernMainWindow(QMainWindow):
    ...
    def __init__(self, username, user_role, login_window):
        ...
        # Icons for maximize/restore should be white/light gray as they are in the dark menu
    ...
    def init_ui(self):
        ...
        # Page Instantiations (Same as before)
        ...
        pages = [
            ...
            self.failed_inventory_report_page,  # 4
            self.outgoing_form_page,  # 5
            self.rrf_page,  # 6
            self.receiving_report_page,  # 7
            self.qc_failed_passed_page,  # 8
            self.qc_excess_page,  # 9
            self.qc_failed_endorsement_page,  # 10
            self.product_delivery_page,  # 11
            self.requisition_logbook_page,  # 12
            self.audit_trail_page,  # 13
            self.user_management_page  # 14
        ]
    ...
    def create_side_menu(self):
        ...
        # Ensure menu toggle icon is white
        ...
        # Menu Buttons (Icons are guaranteed white/light gray via create_menu_button default color)
    ...
    def setup_status_bar(self):
        ...
        # Icons guaranteed via IconProvider - using white/light gray icons on the dark status bar
