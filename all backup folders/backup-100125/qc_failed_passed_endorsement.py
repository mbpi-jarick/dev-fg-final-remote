...
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QTabWidget, QFormLayout, QLineEdit, QDateEdit,
                             ...
                             QDialog, QDialogButtonBox, QInputDialog, QTextEdit, QFrame)  # Added QFrame
...
# --- UPDATED UI CONSTANTS (Harmonized with New Theme) ---
...
PRIMARY_ACCENT_COLOR = COLOR_PRIMARY
DESTRUCTIVE_COLOR = COLOR_DANGER
SECONDARY_ACCENT_COLOR = COLOR_SECONDARY
...
class QCFailedPassedPage(QWidget):
    ...
    def _get_styles(self) -> str:
        return f"""
            ...
            QLineEdit:focus, QDateEdit:focus, QComboBox:focus, QDateTimeEdit:focus {{
            ...
            /* Instruction Box Style (QFrame with class set, or direct styling for QTextEdit) */
            QGroupBox#InstructionGroup QWidget {{
            ...
            /* Secondary Button (Update/Preview/New) */
            ...
            /* Default Button (Manage lists, Cancel) */
    ...
    def init_ui(self):
        ...
        # Icon
        icon_pixmap = fa.icon('fa5s.flask', color=COLOR_PRIMARY).pixmap(QSize(32, 32))
        icon_label = QLabel()
        icon_label.setPixmap(icon_pixmap)
        header_layout.addWidget(icon_label, alignment=Qt.AlignmentFlag.AlignLeft)

        # Title
        title_label = QLabel("QC Failed to Pass Form Management", objectName="PageHeader")
        ...
        self.tab_widget.addTab(self.view_tab, fa.icon('fa5s.list-alt', color=COLOR_PRIMARY), "All Endorsements")
        self.tab_widget.addTab(self.entry_tab, fa.icon('fa5s.file-signature', color=COLOR_PRIMARY), "Endorsement Entry")
    ...
    def _setup_view_tab(self, tab):
        ...
        instruction_label = QLabel(
            "<b>All Reports:</b> Browse, search, update, or delete existing QC Failed to Passed Endorsements. Select a row to view details.")
    ...
    def _setup_deleted_tab(self, tab):
        ...
        instruction_layout = QHBoxLayout(instruction_frame)
        instruction_label = QLabel(
            "<b>Deleted Reports:</b> Records shown here have been soft-deleted. Restoring a record will reverse the previous inventory reversal transactions and re-apply the correct inventory transactions (stock-out from failed, stock-in to FG).")
        instruction_label.setWordWrap(True)
        instruction_layout.addWidget(instruction_label)
        layout.addWidget(instruction_frame)
    ...
    def _setup_view_details_tab(self, tab):
        ...
        instruction_layout = QHBoxLayout(instruction_frame)
        instruction_label = QLabel(
            "<b>View Details:</b> Detailed view of the selected QC Endorsement. View the primary details, lot breakdown, and any associated excess quantities.")
        instruction_label.setWordWrap(True)
        instruction_layout.addWidget(instruction_label)
        ...
        self.view_breakdown_total_label.setStyleSheet("background-color: transparent;")
        ...
        self.view_excess_total_label.setStyleSheet("background-color: transparent;")
    ...
    def _setup_entry_tab(self, tab):
        # Main layout for the Entry Tab
        tab_layout = QVBoxLayout(tab)
        tab_layout.setContentsMargins(10, 10, 10, 10)
        tab_layout.setSpacing(10)

        # Instruction Box (Fixed Height)
        instruction_text = "Use this form to endorse QC Failed material back into FG inventory (Passed). Look up the QC ID to load initial data, then calculate the lot breakdown."
        ...
        instruction_edit.setMaximumHeight(80)  # Fixed height for instructions
        instruction_vbox = QVBoxLayout(instruction_group)
        instruction_vbox.addWidget(instruction_edit)
        tab_layout.addWidget(instruction_group)

        # Splitter for Details (Left) and Preview (Right)
        main_splitter = QSplitter(Qt.Orientation.Horizontal)
        tab_layout.addWidget(main_splitter, 1)  # Give the splitter the remaining space
        ...
        self.is_lot_range_check = QCheckBox("Calculate lots from a range (e.g., 1001A-1010A)")
        ...
        self.breakdown_total_label.setStyleSheet("background-color: transparent;")
        ...
        self.excess_total_label.setStyleSheet("background-color: transparent;")
        ...
        # --- QTAWESOME ICON & LIGHT BUTTON STYLE ---
        self.preview_btn.setIcon(fa.icon('fa5s.search', color=COLOR_SECONDARY))
        ...
        left_layout.addLayout(button_layout)  # Buttons added to the left side bottom
    ...
    # (All other logic and helper methods remain unchanged)
        ...
        try:
            with self.engine.connect() as conn:
                query = text("""
                    ...
                    FROM qcfp_endorsements_primary WHERE is_deleted IS TRUE
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.information(self, "Success",
                                        f"Endorsement {ref_no} has been restored.")
    ...
    def _show_selected_record_in_view_tab(self):
        ...
        try:
            ...
            self.view_breakdown_total_label.setText(
                f"<b>Total: {sum(Decimal(str(item['quantity_kg'])) for item in breakdown):.2f} kg</b>")
            self._populate_view_table(self.view_excess_table, excess, ["Associated Lot", "Excess Qty (kg)"])
            self.view_excess_total_label.setText(
                f"<b>Total: {sum(Decimal(str(item['quantity_kg'])) for item in excess):.2f} kg</b>")
        except Exception as e:
    ...
    def _add_view_detail_row(self, layout, key, value):
        ...
        # Use simple QLabel wrappers for consistency in the QFormLayout, ensuring they respect the QSS.
    ...
    def _clear_form(self):
        ...
        # Reset save button icon to primary (save) color
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, excess_handling_method):
        ...
        elif num_full_lots > 0:
            # Single lot entry, repeat the same lot for all full lots
        ...
        if excess_qty > 0:
            if is_range and excess_handling_method == 'ASSIGN_TO_LAST_IN_RANGE' and range_end_lot:
                ...
                # Calculate new lot number or assign -EXCESS
                last_lot = calculated_lots[-1] if calculated_lots else lot_input.upper()
                match = re.match(r'^(\d+)([A-Z]*)$', last_lot)

                excess_lot_number = f"{last_lot}-EXCESS"  # Default excess name

                if is_range and match:
                    # If it was a range, the new lot should start immediately after the last range lot
                    range_match = re.match(r'^(\d+)([A-Z]*)$', range_end_lot)
                    if range_match:
                        try:
                            next_num = str(int(range_match.group(1)) + 1).zfill(len(range_match.group(1)))
                            suffix = range_match.group(2)
                            excess_lot_number = f"{next_num}{suffix}"
                        except ValueError:
                            pass  # Use default if conversion fails
                elif not is_range and match:
                    # If it was a single lot, increment it (e.g. 1001AA -> 1002AA)
                    try:
                        next_num = str(int(match.group(1)) + 1).zfill(len(match.group(1)))
        ...
        for lot in all_lots_to_log:
            failed_transactions_out.append(
                {"transaction_date": primary_data["endorsement_date"], "transaction_type": "QC_FP_OUT_FROM_FAILED",
                 ...
                 "lot_number": lot["lot_number"], "quantity_in": 0, "quantity_out": lot["quantity_kg"], "unit": "KG.",
                 "warehouse": primary_data["warehouse"], "encoded_by": self.username,
                 "remarks": f"Passed QC. Ref: {sys_ref_no}"})
        if failed_transactions_out:
            conn.execute(text("""INSERT INTO failed_transactions (transaction_date, transaction_type, source_ref_no, product_code, lot_number, quantity_in, quantity_out, unit, warehouse, encoded_by, remarks) 
                VALUES (:transaction_date, :transaction_type, :source_ref_no, :product_code, :lot_number, :quantity_in, :quantity_out, :unit, :warehouse, :encoded_by, :remarks)"""),
                         failed_transactions_out)
        transactions_in = []
        for lot in all_lots_to_log:
            transactions_in.append(
                {"transaction_date": primary_data["endorsement_date"], "transaction_type": "QC_FP_IN_TO_FG",
                 "source_ref_no": sys_ref_no, "product_code": primary_data["product_code"],
                 "lot_number": lot["lot_number"], "quantity_in": lot["quantity_kg"], "quantity_out": 0, "unit": "KG.",
                 "warehouse": primary_data["warehouse"], "encoded_by": self.username,
                 "remarks": f"Received from QC. Ref: {sys_ref_no}"})
        if transactions_in:
            conn.execute(text("""INSERT INTO transactions (transaction_date, transaction_type, source_ref_no, product_code, lot_number, quantity_in, quantity_out, unit, warehouse, encoded_by, remarks) 
                VALUES (:transaction_date, :transaction_type, :source_ref_no, :product_code, :lot_number, :quantity_in, :quantity_out, :unit, :warehouse, :encoded_by, :remarks)"""),
                         transactions_in)
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            # Update Save button icon for update context
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                ...
                failed_trans_in = [
                    ...
                    all_lots]
                if failed_trans_in: conn.execute(text("""INSERT INTO failed_transactions (transaction_date, transaction_type, source_ref_no, product_code, lot_number, quantity_in, quantity_out, unit, warehouse, encoded_by, remarks) 
                    VALUES (:date, :type, :ref, :pcode, :lot, :in, :out, 'KG.', :wh, :user, 'Stock returned on deletion')"""),
                ...
                trans_out = [{"type": "QC_FP_DELETED (RETURN_FROM_FG)", "in": 0, "out": lot['quantity_kg'], **lot} for
                             lot in all_lots]
                if trans_out: conn.execute(text("""INSERT INTO transactions (transaction_date, transaction_type, source_ref_no, product_code, lot_number, quantity_in, quantity_out, unit, warehouse, encoded_by, remarks) 
                    VALUES (:date, :type, :ref, :pcode, :lot, :in, :out, 'KG.', :wh, :user, 'Stock returned on deletion')"""),
                                           [{"date": primary_data['endorsement_date'], "ref": ref_no,
                                             "pcode": primary_data['product_code'], "lot": l['lot_number'], **l,
                                             "wh": primary_data['warehouse'], "user": self.username} for l in
