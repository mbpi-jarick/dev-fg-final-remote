# endorsement.py
# FINAL VERSION - Includes robust, dynamic beginning inventory check.
# MODIFIED - Ensures transaction logs are synced on all CRUD operations.
# MODIFIED - Removed automatic notifications on record loading.
# MODIFIED - Removed focus rectangle ("square") from all tables for a cleaner UI.
# REVISED - Standardized table colors and implemented a more advanced UI with icons.
# REVISED - Set a dark blue background for selected rows in all tables.
...
class ManageListDialog(QDialog):
    def __init__(self, parent, db_engine, table_name, column_name, title):
        ...
        add_btn = QPushButton(self.style().standardIcon(QStyle.StandardPixmap.SP_ListPlus), " Add")
        remove_btn = QPushButton(self.style().standardIcon(QStyle.StandardPixmap.SP_ListMinus), " Remove")
        add_btn.setObjectName("PrimaryButton")
        remove_btn.setObjectName("remove_item_btn")  # Destructive action
...
class FGEndorsementPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Add icons and slightly shorter text to tabs
        self.tab_widget.addTab(view_tab, self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogListView), "All Records")
        self.tab_widget.addTab(self.entry_tab, self.style().standardIcon(QStyle.StandardPixmap.SP_FileIcon), "Entry Form")
        self.tab_widget.addTab(self.view_details_tab, self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView), "View Details")
        self.tab_widget.addTab(self.deleted_tab, self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon), "Deleted")
        ...
        # Apply global stylesheet for all tables within this widget
        self.setStyleSheet("""
            QTableWidget::item:selected {
                background-color: #007BFF; /* Dark blue selection color */
                color: white;
            }
    ...
    def _setup_deleted_tab(self, tab):
        ...
        self.deleted_refresh_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_BrowserReload))
        top_layout.addWidget(self.deleted_refresh_btn)
        self.restore_btn = QPushButton(" Restore Selected")
        self.restore_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogResetButton))
    ...
    def _setup_view_details_tab(self, tab):
        ...
        self.view_breakdown_table.setObjectName("BorderlessTable") # For specific styling
        ...
        self.view_excess_table.setObjectName("BorderlessTable") # For specific styling
    ...
    def _setup_entry_tab(self, tab):
        ...
        self.check_inventory_btn = QPushButton(" Check")
        self.check_inventory_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogApplyButton))
        ...
        self.manage_endorser_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DesktopIcon))
        self.manage_remarks_btn = QPushButton(" Manage")
        self.manage_remarks_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DesktopIcon))
        self.location_combo = QComboBox()
        self.add_location_btn = QPushButton(" Manage")
        self.add_location_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DesktopIcon))
        ...
        self.preview_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView))
        ...
        self.clear_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_FileIcon))
        self.cancel_update_btn = QPushButton(" Cancel Update")
        self.cancel_update_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton))
    ...
    def _setup_view_tab(self, tab):
        ...
        self.update_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))
    ...
    #
    # --- ALL OTHER HELPER METHODS BELOW THIS POINT ARE UNCHANGED ---
    #
    ...
    def _show_records_table_context_menu(self, position):
        ...
        menu = QMenu()
        view_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView), "View Details...")
        edit_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView), "Edit Record")
        delete_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon), "Delete Record")
    ...
    def _show_deleted_table_context_menu(self, position):
        ...
        menu = QMenu()
        restore_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogResetButton), "Restore Record")
        view_action = menu.addAction(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView), "View Details...")
