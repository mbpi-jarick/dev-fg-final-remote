...
# MODIFIED - RRF for RAW MATERIAL is now correctly treated as a quantity_out transaction.
...
# FIX - Stock check now correctly uses the central `transactions` table.
# --- NEW MODIFICATIONS ---
# ADDED - Location selection (WH1-WH4) to the Lot Breakdown Tool.
# CORRECTED - Saving a Lot Breakdown now logs each new lot as a 'quantity_out' record in the central 'transactions' table.
# CORRECTED - Deleting a breakdown now also deletes its corresponding transaction logs.
# --- UI MODERNIZATION REVISION ---
...
# ADDED - Non-modal notification system for user feedback on actions.
# ADDED - Admin password prompt for deleting records.
...
class RRFItemEntryDialog(QDialog):
    def __init__(self, db_engine, material_type, item_data=None, parent=None):
        ...
        if fa: self.manage_units_btn.setIcon(fa.icon('fa5s.cogs', color='#7f8c8d'))
    ...
    def _check_lot_in_inventory(self):
        ...
        try:
            ...
            if current_stock > 0:
                ...
                self.ok_button.setEnabled(True)
                self.quantity_edit.setText(f"{current_stock:.2f}")
            else:
...
class RRFPage(QWidget):
    ...
    def init_ui(self):
        ...
        if fa:
            self.tab_widget.addTab(view_tab, fa.icon('fa5s.table', color='#3498db'), "RRF Records")
            self.tab_widget.addTab(self.entry_tab, fa.icon('fa5s.edit', color='#2ecc71'), "RRF Entry")
            self.tab_widget.addTab(self.view_details_tab, fa.icon('fa5s.eye', color='#9b59b6'), "View RRF Details")
            self.tab_widget.addTab(breakdown_tool_tab, fa.icon('fa5s.sitemap', color='#e67e22'), "Lot Breakdown Tool")
            self.tab_widget.addTab(breakdown_records_tab, fa.icon('fa5s.list-alt', color='#1abc9c'), "Breakdown Records")
            self.tab_widget.addTab(deleted_tab, fa.icon('fa5s.trash-alt', color='#e74c3c'), "Deleted")
        else:
    ...
    def _setup_view_tab(self, tab):
        ...
        if fa:
            self.refresh_records_btn.setIcon(fa.icon('fa5s.sync-alt', color='blue'))
    ...
    def _setup_entry_tab(self, tab):
        ...
        if fa:
            add_item_btn.setIcon(fa.icon('fa5s.plus', color='white'))
        ...
        if fa:
            ...
            self.print_btn.setIcon(fa.icon('fa5s.print', color='#3498db'))
    ...
    def _setup_lot_breakdown_tab(self, tab):
        ...
        params_layout.addWidget(QLabel("Target Warehouse:"), 3, 0)
        params_layout.addWidget(self.breakdown_location_combo, 3, 1, 1, 3)
        ...
        if fa:
            self.breakdown_save_btn.setIcon(fa.icon('fa5s.save', color='white'))
            preview_btn.setIcon(fa.icon('fa5s.search', color='#34495e'))
            clear_btn.setIcon(fa.icon('fa5s.eraser', color='#34495e'))
    ...
    def _setup_breakdown_records_tab(self, tab):
        ...
        if fa:
            self.refresh_breakdown_btn.setIcon(fa.icon('fa5s.sync-alt', color='blue'))
            self.load_breakdown_btn.setIcon(fa.icon('fa5s.upload', color='#34495e'))
            self.delete_breakdown_btn.setIcon(fa.icon('fa5s.trash-alt', color='white'))
    ...
    def _load_breakdown_for_update(self):
        ...
        for i in range(self.tab_widget.count()):
            if "Lot Breakdown Tool" in self.tab_widget.tabText(i):
    ...
    def _delete_selected_breakdown(self):
        ...
        try:
            ...
            self.log_audit_trail("DELETE_RRF_BREAKDOWN", f"Deleted breakdown and transactions for RRF {rrf_no}, Item {item_id}")
    ...
    def _show_records_table_context_menu(self, pos):
        ...
        if fa:
            ...
            print_action = menu.addAction(fa.icon('fa5s.print', color='#1abc9c'), "Print Preview")
            menu.addSeparator()
