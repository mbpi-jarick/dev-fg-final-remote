...
# V3.0 (UI ENHANCEMENT) - Added icons to all buttons/tabs and styled the selected row color.
...
# NEW CLASS: Handles camera feed in a separate thread to prevent GUI freezing.
# In product_delivery.py, replace the entire CameraThread class
...
class CameraThread(QThread):
    ...
    This version includes a more robust resource release to prevent memory errors.
    """
    ...
    def __init__(self, camera_index=0, parent=None):
        ...
        self._is_running = True  # Use a private flag
    ...
    def run(self):
        ...
        while self._is_running:
            ret, frame = cap.read()
            if not ret:
                # If reading the frame fails, stop the loop.
            ...
            try:
                ...
                if decoded_objects:
                    ...
                    # Don't draw on the frame in the background thread to avoid potential issues
            except Exception as e:
                print(f"CameraThread: Error during QR decoding: {e}")

            # --- Frame Conversion ---
            # Check if the thread is still supposed to be running before emitting
            if self._is_running:
                try:
                    ...
                    # Making a copy is safer as the original buffer can be overwritten by the next frame
                    self.frame_ready.emit(qt_image.copy())
                except Exception as e:
                    print(f"CameraThread: Error converting frame: {e}")

            # A small sleep can help prevent 100% CPU usage and make the thread more stable
            self.msleep(10)

        # --- CRITICAL: Ensure release happens in this thread ---
...
class ProductDeliveryPage(QWidget):
    ...
    # ... (Most methods are unchanged, only _setup_scanner_tab and related helpers are new/modified) ...
    ...
    def init_ui(self):
        ...
        style = self.style()
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(view_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(self.view_details_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_MessageBoxInformation))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(entry_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_FileIcon))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(deleted_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(breakdown_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_ToolBarHorizontalExtensionButton))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(scanner_tab),
                                   style.standardIcon(QStyle.StandardPixmap.SP_DialogOkButton))
        self.tab_widget.setTabIcon(self.tab_widget.indexOf(tracking_tab),
    ...
    # In ProductDeliveryPage, replace _toggle_camera and _handle_qr_code

    def _toggle_camera(self):
        # This method is now only for starting and manual stopping
        if hasattr(self, 'camera_thread') and self.camera_thread and self.camera_thread.isRunning():
            self.camera_thread.stop()  # This now just sets the flag
            self.toggle_camera_btn.setEnabled(False)  # Disable while stopping
            ...
            # Connect the finished signal to reset the button
            self.camera_thread.finished.connect(self._on_camera_thread_finished)

            self.camera_thread.start()
            self.toggle_camera_btn.setText("Stop Camera")
            self.toggle_camera_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaStop))
            self.camera_view_label.setText("Starting camera...")

    def _on_camera_thread_finished(self):
        """Slot to handle cleanup after the camera thread has finished."""
        print("MainThread: Camera thread finished.")
        self.toggle_camera_btn.setText("Start Camera")
        self.toggle_camera_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaPlay))
        ...
        # Make sure the reference is cleaned up
    ...
    def _handle_qr_code(self, data: str):
        """
        Slot to handle a detected QR code.
        The camera thread will now stop itself automatically.
        """
        # Only process if the input is empty to prevent multiple triggers
        if not self.scanner_input.text():
            print(f"QR Code Detected: {data}")
            self.scanner_input.setText(data)

            # The camera thread will stop on its own, and the finished signal
            # will reset the UI. We just need to process the data.
            QTimer.singleShot(0, self._update_delivery_status)

    def _update_camera_view(self, image: QImage):
        """Slot to update the QLabel with a new frame from the camera."""
        self.camera_view_label.setPixmap(QPixmap.fromImage(image))

    # Find this method in ProductDeliveryPage and REPLACE it with this new version.
    ...
    def _handle_qr_code(self, data: str):
        ...
        """
        # Only process if the QR code is new or the input is empty
        if self.scanner_input.text() != data:
            print(f"QR Code Detected: {data}")
            self.scanner_input.setText(data)

            # --- THE CRITICAL FIX ---
            # Instead of calling the update function directly, we schedule it to run
            # as soon as the application is ready. This safely detaches the action
            # from the background thread's signal.
            QTimer.singleShot(0, self._update_delivery_status)
            # ------------------------

            # Stop the camera for a better user experience after a successful scan
            if hasattr(self, 'camera_thread') and self.camera_thread and self.camera_thread.isRunning():
                self._toggle_camera()
            ...
            try:
                ...
                if decoded_objects:
                    qr_data = decoded_objects[0].data.decode('utf-8')
                    self.qr_code_detected.emit(qr_data)
                    # --- NEW: Tell the thread to stop itself ---
                    self._is_running = False
            except Exception as e:
                print(f"CameraThread: Error during QR decoding: {e}")

    def _show_camera_error(self, message: str):
        """Slot to display a camera error message."""
    ...
    # In ProductDeliveryPage, replace your _setup_scanner_tab method
    ...
    def _setup_scanner_tab(self, tab):
        ...
        scan_btn = QPushButton("Update Status Manually")
        scan_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogOkButton))
        ...
        form_layout.addRow(self.scanner_status_label)  # Add the label to the layout
        # --- END NEW ---
        ...
        clear_log_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
    ...
    def _setup_lot_breakdown_tab(self, tab):
        ...
        self.breakdown_save_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton))
        preview_btn = QPushButton("Preview Breakdown")
        preview_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))
    ...
    def _setup_view_tab(self, tab):
        ...
        self.update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp))
    ...
    def _setup_entry_tab(self, tab):
        ...
        add_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogApplyButton))
        self.edit_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView))
        self.remove_item_btn = QPushButton("Remove Item")
        self.remove_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton))
        ...
        self.cancel_update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton))
        self.clear_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileIcon))
        ...
        self.print_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))
    ...
    def _on_tab_changed(self, index):
        ...
        # --- NEW: Manage the camera thread ---
        if tab_text != "QR Scanner":
            if hasattr(self, 'camera_thread') and self.camera_thread and self.camera_thread.isRunning():
                self._toggle_camera()  # This will call stop() and update the UI
        # --- END OF NEW BLOCK ---
    ...
    def _on_record_selection_changed(self):
        ...
        is_printed = selected and self.records_table.item(selected[0].row(), self.records_table.columnCount() - 1).data(
    ...
    # ... The rest of your methods (_show_selected_record_in_view_tab, _add_view_detail_row, etc.)
    # can remain exactly as they were in your original file. I have omitted them here for brevity,
    # but you should keep them in your final file.
    # The only changes are the ones detailed above: new imports, the CameraThread class,
    # the modified _setup_scanner_tab, the new camera helper methods, and the update to _on_tab_changed.
        ...
        try:
            with self.engine.connect() as conn:
                ...
                res = conn.execute(text(f"""
                    SELECT p.dr_no, p.delivery_date, p.customer_name, p.order_form_no, p.is_printed,
                           STRING_AGG(i.product_code, ', ') as product_codes, SUM(i.quantity) as total_quantity
    ...
    def _show_records_table_context_menu(self, pos):
        ...
        view_dr_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))

        view_details_action = menu.addAction("View Record Details");
        view_details_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_MessageBoxInformation))
        menu.addSeparator()
        edit_action = menu.addAction("Edit Record");
        edit_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp))
        delete_action = menu.addAction("Delete Record");
        delete_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
    ...
    def _show_deleted_table_context_menu(self, pos):
        ...
        restore_action.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogResetButton))
    ...
    def _load_tracking_data(self):
        ...
        try:
            with self.engine.connect() as conn:
                sql = text(
                    "SELECT dt.dr_no, p.customer_name, dt.status, dt.scanned_by, dt.scanned_on FROM delivery_tracking dt JOIN product_delivery_primary p ON dt.dr_no = p.dr_no WHERE dt.dr_no ILIKE :search ORDER BY dt.scanned_on DESC")
    ...
    # In ProductDeliveryPage, replace your _update_delivery_status method

    # In ProductDeliveryPage, replace this entire method
    ...
    def _update_delivery_status(self):
        ...
        try:
            with self.engine.connect() as conn:
                with conn.begin():
                    # Step 1: Check if the primary DR record exists at all
                    ...
                    # --- NEW LOGIC: Step 2: Check if it's already scanned ---
                    ...
                    if existing_status is not None:
                        # The record already exists in the tracking table.
                        self.scanner_status_label.setText(
                            f"<font color='orange'>Status: DR #{dr_no} ALREADY SCANNED ({existing_status}).</font>")
                        # Add to local log table for user feedback, but don't update DB again
                    ...
                    # Step 3: If we reach here, the DR exists and has not been scanned yet. Proceed with the update.
                    upsert_sql = text("""
                        INSERT INTO delivery_tracking (dr_no, status, scanned_by, scanned_on)
                        VALUES (:dr_no, :status, :user, :now)
                    """)  # No need for ON CONFLICT anymore, since we check first
...
if __name__ == "__main__":
    ...
    try:
        with mock_engine.connect() as conn, conn.begin():
            conn.execute(text(
                "CREATE TABLE product_delivery_primary (id INTEGER PRIMARY KEY, dr_no TEXT UNIQUE, dr_type TEXT, delivery_date TEXT, customer_name TEXT, deliver_to TEXT, address TEXT, po_no TEXT, order_form_no TEXT, terms TEXT, prepared_by TEXT, encoded_by TEXT, encoded_on TEXT, edited_by TEXT, edited_on TEXT, is_deleted BOOLEAN DEFAULT FALSE, is_printed BOOLEAN DEFAULT FALSE);"))
            conn.execute(text(
                "CREATE TABLE product_delivery_items (id INTEGER PRIMARY KEY, dr_no TEXT, quantity REAL, unit TEXT, product_code TEXT, product_color TEXT, no_of_packing INTEGER, weight_per_pack REAL, attachments TEXT, unit_price REAL, lot_no_1 TEXT, lot_no_2 TEXT, lot_no_3 TEXT);"))
            conn.execute(text(
            ...
            conn.execute(text(
                "CREATE TABLE delivery_tracking (dr_no TEXT UNIQUE, status TEXT, scanned_by TEXT, scanned_on TEXT);"))
