...
# --- MODIFIED: Worker Object for Threading (by Lot Number) ---
class InventoryWorker(QObject):
    ...
    def run(self):
        try:
            # --- Query now only selects and groups by lot_number ---
            query_str = """
                SELECT
                    lot_number,
...
class GoodInventoryPage(QWidget):
    ...
    def init_ui(self):
        ...
        layout.setContentsMargins(20, 20, 20, 20)  # Corrected from setContentsContents

        # --- MODIFIED: Filters & Controls Group (by Lot Number) ---
        ...
        # --- MODIFIED: Table Widget (2 Columns) ---
    ...
    def _display_inventory(self, df, is_specific_lot):
        self.inventory_table.setRowCount(0)
        self.inventory_table.setSpan(0, 0, 1, 1)  # Reset span
