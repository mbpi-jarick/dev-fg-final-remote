...
# --- NEW IMPORT: GOOD INVENTORY PAGE ---
from good_inventory_page import GoodInventoryPage

# ---------------------------------------

# --- CONFIGURATION ---
DB_CONFIG = {"host": "localhost", "port": 5432, "dbname":
...
class AppStyles:
    ...
    MAIN_WINDOW_STYLESHEET = f"""
        ...
        #SideMenu #MenuLabel {{
            ...
            color: #95a5a6; padding: 10px 10px 4px 15px;
            margin-top: 8px; border-top: 1px solid #34495e;
        }}
        #SideMenu #MenuLabel:first-of-type {{ border-top: none; }}

        /* Side menu buttons - updated for icons */
        #SideMenu QPushButton {{
            background-color: transparent;
            color: #ecf0f1;
            border: none;
            padding: 10px 10px 10px 20px; /* CHANGED: Consistent vertical padding */
            text-align: left;
            font-size: 10pt; /* CHANGED: Slightly larger for clarity */
            font-weight: normal;
            border-radius: 6px;
            qproperty-iconSize: 16px; /* CHANGED: Set a consistent icon size */
        }}
        ...
        QPushButton {{
            border: none;
            /* CHANGED: Adjusted padding to better accommodate icons */
            padding: 9px 16px;
            border-radius: 6px;
            font-weight: bold;
            color: white;
            background-color: {NEUTRAL_COLOR}; /* Default to neutral gray */
            qproperty-iconSize: 16px; /* CHANGED: Set consistent icon size */
        }}
        ...
        QTabWidget::pane {{
            border: 1px solid #e0e5eb; border-radius: 8px;
            background-color: #ffffff; padding: 10px; margin-top: -1px;
        }}
        QTabBar {{
            qproperty-drawBase: 0; background-color: transparent; margin-bottom: 0px;
        }}
        QTabBar::tab {{
            background-color: #e9eff7; color: {NEUTRAL_COLOR}; padding: 10px 25px;
            border-top-left-radius: 8px; border-top-right-radius: 8px;
            border: 1px solid #e0e5eb; border-bottom: none; margin-right: 4px;
            font-weight: bold;
        }}
        QTabBar::tab:selected {{
            color: {PRIMARY_ACCENT_COLOR}; background-color: #ffffff;
            border: 1px solid #e0e5eb; border-bottom-color: #ffffff;
            margin-bottom: -1px;
        }}
        ...
        /* === StatusBar Styling === */
        QStatusBar {{
            background-color: #e9ecef; color: #333; font-size: 9pt; padding: 2px 0px;
        }}
...
class NetworkGraphWidget(QWidget):
    # ... (NetworkGraphWidget code remains unchanged) ...
...
# ... (SyncWorker, SyncCustomerWorker, SyncDeliveryWorker, SyncRRFWorker classes remain unchanged) ...
...
class LoginWindow(QMainWindow):
    # ... (LoginWindow code remains unchanged) ...
...
class ModernMainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Page Instantiations (INDICES ADJUSTED)
        ...
        # --- NEW INVENTORY PAGE INSTANTIATION ---
        ...
        # Add pages to Stacked Widget in order (INDICES ADJUSTED)
        pages = [
            self.fg_endorsement_page,  # 0
            self.transactions_page,  # 1
            self.failed_transactions_page,  # 2
            self.good_inventory_page,  # 3 <-- NEW PAGE
            self.outgoing_form_page,  # 4 (Shifted from 3)
            self.rrf_page,  # 5 (Shifted from 4)
            self.receiving_report_page,  # 6 (Shifted from 5)
            self.qc_failed_passed_page,  # 7 (Shifted from 6)
            self.qc_excess_page,  # 8 (Shifted from 7)
            self.qc_failed_endorsement_page,  # 9 (Shifted from 8)
            self.product_delivery_page,  # 10 (Shifted from 9)
            self.requisition_logbook_page,  # 11 (Shifted from 10)
            self.audit_trail_page,  # 12 (Shifted from 11)
            self.user_management_page  # 13 (Shifted from 12)
        ]
        for page in pages:
    ...
    def create_side_menu(self):
        ...
        # --- NEW INVENTORY BUTTON (INDEX 3) ---
        self.btn_good_inventory = self.create_menu_button("  FG Inventory Report", IconProvider.GOOD_INVENTORY, 3)

        # --- SHIFTED INDICES START HERE ---
        self.btn_outgoing_form = self.create_menu_button("  Outgoing Form", IconProvider.OUTGOING_FORM, 4)
        self.btn_rrf = self.create_menu_button("  RRF Form", IconProvider.RRF_FORM, 5)
        self.btn_receiving_report = self.create_menu_button("  Receiving Report", IconProvider.RECEIVING_REPORT, 6)
        self.btn_qc_failed_passed = self.create_menu_button("  QC Failed->Passed", IconProvider.QC_PASSED, 7)
        self.btn_qc_excess = self.create_menu_button("  QC Excess", IconProvider.QC_EXCESS, 8)
        self.btn_qc_failed = self.create_menu_button("  QC Failed", IconProvider.QC_FAILED, 9)
        self.btn_product_delivery = self.create_menu_button("  Product Delivery", IconProvider.PRODUCT_DELIVERY, 10)
        self.btn_requisition_logbook = self.create_menu_button("  Requisition Logbook", IconProvider.REQUISITION, 11)
        ...
        self.btn_audit_trail = self.create_menu_button("  Audit Trail", IconProvider.AUDIT_TRAIL, 12)
        self.btn_user_mgmt = self.create_menu_button("  User Management", IconProvider.USER_MANAGEMENT, 13)
        # --- RE-INDEXED MENU BUTTONS (END) ---
    ...
    def create_status_widget(self, icon_name, initial_text, icon_color='#6c757d'):
        ...
        layout.setSpacing(5)
        icon_label = QLabel()
        pixmap = IconProvider.get_pixmap(icon_name, icon_color, QSize(12, 12))
    ...
    def update_sync_progress(self, percent):
        """Updates the progress label in the loading dialog."""
        if hasattr(self, 'loading_dialog') and self.loading_dialog.isVisible():
            self.loading_dialog.progress_label.setText(f"{percent}%")
    ...
    def closeEvent(self, event):
        ...
        should_close = self.prompt_on_close()
    ...
    def prompt_on_close(self):
        ...
        if dialog.clickedButton() == logout_button:
            ...
            self.log_audit_trail("LOGOUT", "User exited application via close button.")
