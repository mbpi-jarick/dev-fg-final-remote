...
import sys  # For application exit handling

# SQLAlchemy Imports
from sqlalchemy import create_engine, text, Engine, MetaData, Table, Column, String, Float, insert, exc
from sqlalchemy.engine import URL  # Needed for connection
...
# --- FAILED INVENTORY DASHBOARD WIDGET (Unchanged) ---
class FailedDashboardWidget(QWidget):
    ...
    def _create_summary_label(self, title: str, initial_value: str) -> QWidget:
        ...
        widget.setStyleSheet("border: 1px solid #C9302C; border-radius: 5px; background-color: #fef0f0;")
        return widget
    ...
    def _create_lot_statistics_group(self) -> QGroupBox:
        ...
        layout.addWidget(QLabel("Largest Failed Lot (kg):"), 0, 0)
        layout.addWidget(self.lot_stats['max_lot'], 0, 1)

        layout.addWidget(QLabel("Smallest Failed Lot (kg):"), 1, 0)
        layout.addWidget(self.lot_stats['min_lot'], 1, 1)

        layout.addWidget(QLabel("Average Failed Lot (kg):"), 2, 0)
        layout.addWidget(self.lot_stats['avg_lot'], 2, 1)

        layout.addWidget(QLabel("Median Failed Lot (kg):"), 3, 0)
        layout.addWidget(self.lot_stats['median_lot'], 3, 1)

        for label in self.lot_stats.values():
            label.setStyleSheet("font-weight: bold; color: #C9302C;")
    ...
    def update_dashboard(self, df: pd.DataFrame):
        ...
        if df.empty or df['current_balance'].sum() == 0:
            for container_widget in [self.total_lots_label, self.total_products_label, self.overall_balance_label]:
                ...
                if value_label:
                    is_lot_count = 'Lots' in container_widget.findChild(QLabel, "TitleLabel").text()
        ...
        for i, row in product_summary.iterrows():
            ...
            progress_bar.setStyleSheet("""
                ...
                QProgressBar::chunk { background-color: #C9302C; } 
            """)
...
class FailedInventoryReportPage(QWidget):
    ...
    def _get_styles(self):
        return """
            ...
            QTableWidget { 
                ...
                alternate-background-color: #fffafa; 
            ...
            #TotalBalanceLabel { color: #C9302C; }
            #PrimaryButton { 
                background-color: #C9302C; 
                color: white; 
                border-radius: 4px;
            ...
            #PrimaryButton:hover { background-color: #a94442; }
            QTextEdit#InstructionsBox { background-color: #fef0f0; border: 1px solid #C9302C; padding: 5px; } 
        """
    ...
    def init_ui(self):
        ...
        self.tab_widget.addTab(
            self.inventory_tab,
            qta.icon('fa5s.bomb', color='#C9302C'),
            "Failed Inventory Details"
        )

        self.tab_widget.addTab(
            self.dashboard_widget,
            qta.icon('fa5s.chart-line', color='#C9302C'),
            "Advanced Dashboard"
        )
...
if __name__ == '__main__':
    ...
    main_window.setWindowTitle("Inventory System - Failed Inventory Report")
