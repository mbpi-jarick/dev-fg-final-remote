...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QLabel, QLineEdit, QPushButton,
                             ...
                             QFrame, QStatusBar, QDialog, QGraphicsDropShadowEffect)  # Added QGraphicsDropShadowEffect
...
from transactions_form import TransactionsFormPage  # <--- IMPORT THE NEW FORM
...
class AppStyles:
    ...
    BORDER_COLOR = "#e0e5eb"
    SIDE_MENU_BG = "#2c3e50"
    SIDE_MENU_HOVER_BG = "#34495e"
    SIDE_MENU_TEXT = "#ecf0f1"
    TEXT_COLOR_DARK = "#333"
    TEXT_COLOR_LIGHT = "#6c757d"

    LOGIN_STYLESHEET = f"""
        #LoginWindow, #FormFrame {{ background-color: {BACKGROUND_COLOR}; }}
    ...
    MAIN_WINDOW_STYLESHEET = f"""
        ...
        /* === Side Menu Styling (Revamped) === */
        QWidget#SideMenu {{
            background-color: {SIDE_MENU_BG};
            color: {SIDE_MENU_TEXT};
            border-right: 1px solid {SIDE_MENU_HOVER_BG};
        }}
        ...
        #SideMenu QPushButton {{
            ...
            font-size: 10pt;
            border-radius: 6px;
            margin: 2px 8px; /* Give buttons some breathing room */
            transition: background-color 0.2s ease-in-out;
        }}
        #SideMenu QPushButton:hover {{ background-color: {SIDE_MENU_HOVER_BG}; }}
        #SideMenu QPushButton:checked {{
            ...
            padding-left: 16px; /* Adjust padding to make space for the border */
        }}

        /* Profile Section */
        #SideMenu #ProfileName {{ font-weight: bold; font-size: 11pt; color: white; }}
        ...
        #ShadowFrame {{
            ...
            margin: 0px 15px 15px 15px; /* Margin around the shadow frame within the main content area */
        }}
        QGroupBox {{
            border: 1px solid {BORDER_COLOR}; border-radius: 8px;
            margin-top: 12px;
            /* Note: GroupBox background is now transparent to inherit from ShadowFrame */
            background-color: transparent; 
        }}
        QGroupBox::title {{
            subcontrol-origin: margin; subcontrol-position: top left;
            padding: 2px 10px; background-color: {BACKGROUND_COLOR};
            border: 1px solid {BORDER_COLOR}; border-bottom: none;
            border-top-left-radius: 8px; border-top-right-radius: 8px;
            font-weight: bold; color: {TEXT_COLOR_LIGHT};
        }}
        QLineEdit, QComboBox, QDateEdit, QDateTimeEdit, QPlainTextEdit, QDoubleSpinBox, QTextEdit {{
            ...
            background-color: {CONTENT_BACKGROUND_COLOR};
            selection-background-color: {PRIMARY_ACCENT_LIGHT};
        }}
        QLineEdit:focus, QComboBox:focus, QDateEdit:focus, QPlainTextEdit:focus, QDoubleSpinBox:focus, QTextEdit:focus {{
            ...
            padding: 7px; /* Adjust padding to keep size consistent */
        }}
        QLineEdit[readOnly="true"] {{ background-color: #eff2f7; color: {TEXT_COLOR_LIGHT}; }}
        ...
        /* === Table Styling === */
        QTableWidget {{
            border: none; background-color: {CONTENT_BACKGROUND_COLOR};
            selection-behavior: SelectRows;
        }}
        QTableWidget::item {{ border-bottom: 1px solid {BACKGROUND_COLOR}; padding: 10px; }}
        QTableWidget::item:selected {{ background-color: {PRIMARY_ACCENT_LIGHT}; color: {TEXT_COLOR_DARK}; }}
        QHeaderView::section {{
            background-color: {CONTENT_BACKGROUND_COLOR}; color: {TEXT_COLOR_LIGHT}; padding: 10px;
            border: none; border-bottom: 2px solid {BORDER_COLOR};
            font-weight: bold; text-transform: uppercase;
        }}

        /* === Custom ScrollBar Styling === */
        QScrollBar:vertical {{
            border: none; background: {BACKGROUND_COLOR}; width: 10px; margin: 0px 0px 0px 0px;
        }}
        QScrollBar::handle:vertical {{
            background: #c0c0c0; min-height: 20px; border-radius: 5px;
        }}
        QScrollBar::handle:vertical:hover {{ background: #a0a0a0; }}
        QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical {{ height: 0px; }}

        QScrollBar:horizontal {{
            border: none; background: {BACKGROUND_COLOR}; height: 10px; margin: 0px 0px 0px 0px;
        }}
        QScrollBar::handle:horizontal {{
            background: #c0c0c0; min-width: 20px; border-radius: 5px;
        }}
        ...
        QStatusBar QLabel {{ color: {TEXT_COLOR_DARK}; background: transparent; padding: 0 4px; }}
    """


# --- NEW: Custom Widgets for Advanced UI ---
class PageHeader(QWidget):
    """A custom widget for consistent page headers."""
    ...
    def __init__(self, title, icon_name, parent=None):
        ...
        self.icon_label.setPixmap(fa.icon(icon_name, color="#333").pixmap(QSize(22, 22)))
...
class ShadowFrame(QFrame):
    """A QFrame that applies a drop shadow effect to its contents."""
    ...
    try:
        with engine.connect() as connection:
            with connection.begin():
                ...
                # --- NEW: Central Transactions Table for Inventory ---
                connection.execute(text("""
                    CREATE TABLE IF NOT EXISTS transactions (
                        id SERIAL PRIMARY KEY,
                        transaction_date DATE NOT NULL,
                        transaction_type VARCHAR(50) NOT NULL, -- e.g., 'FG_ENDORSEMENT', 'PRODUCT_DELIVERY'
                        source_ref_no VARCHAR(50),             -- e.g., System Ref No, DR No
...
class ModernMainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # --- MODIFIED: Page Management ---
        self.page_container = QWidget()
        page_layout = QVBoxLayout(self.page_container)
        page_layout.setContentsMargins(0, 0, 0, 0)  # No margin inside the page container
        page_layout.setSpacing(0)  # No spacing between header and content frame

        # This will be our dynamic header
        self.page_header = PageHeader("Dashboard", "fa5s.tachometer-alt")  # Initial dummy header

        page_layout.addWidget(self.page_header)

        # Use a ShadowFrame to contain the main content
        content_frame = ShadowFrame()
        content_layout = QVBoxLayout(content_frame)
        content_layout.setContentsMargins(15, 15, 15, 15)  # Padding inside the shadow frame for the stacked widget

        self.stacked_widget = QStackedWidget()
        content_layout.addWidget(self.stacked_widget)

        page_layout.addWidget(content_frame, 1)  # Add with stretch factor
        main_layout.addWidget(self.page_container, 1)  # Add with stretch factor

        # Create a dictionary to hold page info for easy access
        self.pages = {
            0: {"widget": FGEndorsementPage(engine, self.username, self.log_audit_trail), "title": "FG Endorsement",
                "icon": "fa5s.file-signature"},
            1: {"widget": TransactionsFormPage(engine, self.username, self.log_audit_trail), "title": "Log Transaction",
                "icon": "fa5s.exchange-alt"},
            2: {"widget": OutgoingFormPage(engine, self.username, self.log_audit_trail), "title": "Outgoing Form",
                "icon": "fa5s.sign-out-alt"},
            3: {"widget": RRFPage(engine, self.username, self.log_audit_trail), "title": "RRF Form",
                "icon": "fa5s.undo-alt"},
            4: {"widget": ReceivingReportPage(engine, self.username, self.log_audit_trail), "title": "Receiving Report",
                "icon": "fa5s.truck-loading"},
            5: {"widget": QCFailedPassedPage(engine, self.username, self.log_audit_trail),
                "title": "QC Failed to Passed", "icon": "fa5s.flask"},
            6: {"widget": QCExcessEndorsementPage(engine, self.username, self.log_audit_trail), "title": "QC Excess",
                "icon": "fa5s.box"},
            7: {"widget": QCFailedEndorsementPage(engine, self.username, self.log_audit_trail), "title": "QC Failed",
                "icon": "fa5s.times-circle"},
            8: {"widget": ProductDeliveryPage(engine, self.username, self.log_audit_trail), "title": "Product Delivery",
                "icon": "fa5s.truck"},
            9: {"widget": RequisitionLogbookPage(engine, self.username, self.log_audit_trail),
                "title": "Requisition Logbook", "icon": "fa5s.book"},
            10: {"widget": AuditTrailPage(engine), "title": "Audit Trail", "icon": "fa5s.clipboard-list"},
            11: {"widget": UserManagementPage(engine, self.username, self.log_audit_trail), "title": "User Management",
                 "icon": "fa5s.users-cog"},
        }

        # Add pages from the dictionary to the stacked widget
        # Ensure they are added in numerical order of their keys
        for index in sorted(self.pages.keys()):
            self.stacked_widget.addWidget(self.pages[index]["widget"])
        ...
        # Show the first page and update header
    ...
    def create_side_menu(self):
        ...
        def create_menu_label(text):
            label = QLabel(text, objectName="MenuLabel")
            return label

        layout.addWidget(create_menu_label("Forms"))
        ...
        self.btn_outgoing_form = self.create_menu_button("  Outgoing Form", 'fa5s.sign-out-alt', 2)
        self.btn_rrf = self.create_menu_button("  RRF Form", 'fa5s.undo-alt', 3)
        self.btn_receiving_report = self.create_menu_button("  Receiving Report", 'fa5s.truck-loading', 4)
        self.btn_qc_failed_passed = self.create_menu_button("  QC Failed->Passed", 'fa5s.flask', 5)
        self.btn_qc_excess = self.create_menu_button("  QC Excess", 'fa5s.box', 6)
        self.btn_qc_failed = self.create_menu_button("  QC Failed", 'fa5s.times-circle', 7)
        self.btn_product_delivery = self.create_menu_button("  Product Delivery", 'fa5s.truck', 8)
        self.btn_requisition_logbook = self.create_menu_button("  Requisition Logbook", 'fa5s.book', 9)
        ...
        layout.addWidget(create_menu_label("Data Sync"))
        ...
        layout.addWidget(create_menu_label("System"))
        self.btn_audit_trail = self.create_menu_button("  Audit Trail", 'fa5s.clipboard-list', 10)
        self.btn_user_mgmt = self.create_menu_button("  User Management", 'fa5s.users-cog', 11)

        layout.addWidget(self.btn_audit_trail);
        layout.addWidget(self.btn_user_mgmt)

        layout.addStretch(1);  # This pushes the final buttons to the bottom
    ...
    def on_customer_sync_finished(self, success, message):
        ...
        if success:
            ...
            if hasattr(self.pages[8]["widget"], '_load_combobox_data'):  # ProductDeliveryPage is index 8
                self.pages[8]["widget"]._load_combobox_data()
            if hasattr(self.pages[3]["widget"], '_load_combobox_data'):  # RRFPage is index 3
                self.pages[3]["widget"]._load_combobox_data()
        else:
            QMessageBox.critical(self, "Sync Result", message);
    ...
    def on_delivery_sync_finished(self, success, message):
        ...
        if success:
            ...
            if self.stacked_widget.currentWidget() == self.pages[8]["widget"]:  # If current page is ProductDeliveryPage
                self.pages[8]["widget"]._load_all_records()
        else:
            QMessageBox.critical(self, "Sync Result", message);
    ...
    def on_rrf_sync_finished(self, success, message):
        ...
        if success:
            ...
            if self.stacked_widget.currentWidget() == self.pages[3]["widget"]:  # If current page is RRFPage
                self.pages[3]["widget"]._load_all_records()
        else:
            QMessageBox.critical(self, "Sync Result", message);
    ...
    def show_page(self, index):
        ...
        page_info = self.pages.get(index)
        if not page_info:
            print(f"Error: Page index {index} not found.")
            return

        # Update the header
        self.page_header.title_label.setText(page_info["title"])
        self.page_header.icon_label.setPixmap(fa.icon(page_info["icon"], color="#333").pixmap(QSize(22, 22)))

        self.stacked_widget.setCurrentIndex(index)
        current_widget = self.stacked_widget.widget(index)

        # These generic calls ensure that any page with these methods will be refreshed
