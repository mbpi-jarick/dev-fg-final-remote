...
class QCFailedPassedPage(QWidget):
    ...
    def _setup_dashboard_tab(self, tab):
        ...
        recent_group = QGroupBox("Recent QC Endorsements")
        recent_layout = QVBoxLayout(recent_group)
        self.dashboard_recent_table = QTableWidget()
        self._configure_table_ui(self.dashboard_recent_table)
            ...
            with self.engine.connect() as conn, conn.begin():
                if is_update:
                    ...
                    conn.execute(text("DELETE FROM qcfp_endorsements_excess WHERE system_ref_no = :ref"),
                                 {"ref": sys_ref_no})
                    update_sql = text(
                        "UPDATE qcfp_endorsements_primary SET form_ref_no=:form_ref_no, endorsement_date=:endorsement_date, product_code=:product_code, lot_number=:lot_number, quantity_kg=:quantity_kg, weight_per_lot=:weight_per_lot, endorsed_by=:endorsed_by, warehouse=:warehouse, received_by_name=:received_by_name, received_date_time=:received_date_time, edited_by=:edited_by, edited_on=:edited_on WHERE system_ref_no = :system_ref_no")
                    conn.execute(update_sql, primary_data);
                    self.log_audit_trail("UPDATE_QCFP_ENDORSEMENT", f"Updated QCFP: {sys_ref_no}");
                    action_text = "updated"
                else:
                    insert_sql = text(
                        "INSERT INTO qcfp_endorsements_primary (system_ref_no, form_ref_no, endorsement_date, product_code, lot_number, quantity_kg, weight_per_lot, endorsed_by, warehouse, received_by_name, received_date_time, encoded_by, encoded_on, edited_by, edited_on) VALUES (:system_ref_no, :form_ref_no, :endorsement_date, :product_code, :lot_number, :quantity_kg, :weight_per_lot, :endorsed_by, :warehouse, :received_by_name, :received_date_time, :encoded_by, :encoded_on, :edited_by, :edited_on)")
                ...
                if breakdown_lots: conn.execute(text(
                ...
                if excess_lots: conn.execute(text(
    ...
    def _delete_record(self):
        ...
        reply = QMessageBox.question(self, "Confirm Deletion",
                                     ...
                                     "This will move it to the 'Deleted' tab.",
                                     QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        ...
        try:
            ...
            QMessageBox.information(self, "Success", f"Endorsement {ref_no} has been moved to the Deleted tab.")
            self._refresh_all_data_views()
        except Exception as e:
        ...
        try:
            with self.engine.connect() as conn:
                endorsed_today = conn.execute(text(
                    "SELECT COUNT(*) FROM qcfp_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                qty_today = conn.execute(text(
                    "SELECT SUM(quantity_kg) FROM qcfp_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or Decimal(
                    '0.00')
                products_today = conn.execute(text(
                    "SELECT COUNT(DISTINCT product_code) FROM qcfp_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                lots_today = conn.execute(text(
                    "SELECT COUNT(*) FROM qcfp_endorsements_secondary s JOIN qcfp_endorsements_primary p ON s.system_ref_no = p.system_ref_no WHERE p.endorsement_date = CURRENT_DATE AND p.is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                recent_endorsements = conn.execute(text(
                    "SELECT system_ref_no, product_code, endorsement_date FROM qcfp_endorsements_primary WHERE is_deleted IS NOT TRUE ORDER BY id DESC LIMIT 5")).mappings().all()
                top_products = conn.execute(text(
                    "SELECT product_code, SUM(quantity_kg) as total_quantity FROM qcfp_endorsements_primary WHERE is_deleted IS NOT TRUE GROUP BY product_code ORDER BY total_quantity DESC LIMIT 5")).mappings().all()
            ...
            for row, record in enumerate(recent_endorsements):
                ...
                self.dashboard_recent_table.setItem(row, 2, QTableWidgetItem(
                    QDate(record['endorsement_date']).toString("yyyy-MM-dd")))
            ...
            if not top_products: self.product_chart.setTitle(
                "Top 5 Products by Total Endorsed Quantity (No Data)"); return
            self.product_chart.setTitle("Top 5 Products by Total Endorsed Quantity")
