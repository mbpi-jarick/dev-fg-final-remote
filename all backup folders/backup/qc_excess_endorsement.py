...
# FIX - Corrected missing 'date' import causing a crash on loading records.
# ENHANCED - Upgraded dashboard chart to an interactive pie chart and removed dashboard table borders.
# REFACTOR - Removed all logic for locating records in the secondary QC database.
# FEATURE - Added database management for Bag Number, Box Number, and Remarks dropdowns.
# USER_FEATURES_V2 - Implemented Deleted tab, restore, deletion password, required fields, and lot breakdown choice.
# USER_FEATURES_V4 - Removed sorting on View Details tables and verified UI selection/focus behavior.
# USER_FEATURES_V5 - Removed borders from tables in the "View Endorsement Details" tab.
...
class QCExcessEndorsementPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.deleted_tab = QWidget() # New Tab for Deleted Records
        ...
        self.tab_widget.addTab(self.deleted_tab, "Deleted Records") # Add tab to widget
        ...
        self._setup_deleted_tab(self.deleted_tab) # Setup the new tab UI
    ...
    def _setup_dashboard_tab(self, tab):
        ...
        (self.kpi_lots_card, self.kpi_lots_value) = self._create_kpi_card("0", "Lots Processed Today")
        ...
        recent_group = QGroupBox("Recent QC Excess Endorsements")
    ...
    def _setup_deleted_tab(self, tab):
        ...
        self.deleted_records_table.itemSelectionChanged.connect(lambda: self.restore_btn.setEnabled(bool(self.deleted_records_table.selectedItems())))
    ...
    def _setup_view_details_tab(self, tab):
        ...
        # MODIFIED: Removed borders and grid lines
    ...
    def _populate_view_table(self, table, data, headers):
        ...
        if not data:
            # Ensure table stretches even when empty
        ...
        # Auto-size columns based on content and stretch the last one
    ...
    def _setup_entry_tab(self, tab):
        ...
        grid_layout.addWidget(self.lot_number_edit, 3, 1, 1, 2);
        grid_layout.addWidget(self.is_lot_range_check, 3, 3)
        ...
        grid_layout.addWidget(QLabel("Bag Number:"), 5, 0);
        grid_layout.addLayout(self._create_combo_with_add("qce_bag_numbers", self.bag_number_combo), 5, 1)
        grid_layout.addWidget(QLabel("Box Number:"), 5, 2);
        grid_layout.addLayout(self._create_combo_with_add("qce_box_numbers", self.box_number_combo), 5, 3)
        grid_layout.addWidget(QLabel("Remarks:"), 6, 0);
        grid_layout.addLayout(self._create_combo_with_add("qce_remarks", self.remarks_combo), 6, 1, 1, 3)
        grid_layout.addWidget(QLabel("<b>Endorsed By:</b>"), 7, 0);
        grid_layout.addLayout(self._create_combo_with_add("qce_endorsers", self.endorsed_by_combo), 7, 1)
        grid_layout.addWidget(QLabel("<b>Received By:</b>"), 7, 2);
        grid_layout.addLayout(self._create_combo_with_add("qce_receivers", self.received_by_combo), 7, 3)
        grid_layout.addWidget(QLabel("Date/Time Received:"), 8, 0);
        grid_layout.addWidget(self.received_date_time_edit, 8, 1, 1, 3)
    ...
    def _generate_preview_from_data(self, record_data):
        ...
        calculation_data = self._perform_lot_calculation(
            ...
            is_update=True # bypass dialog on load
        )
        if calculation_data:
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, is_update=False):
        ...
        if excess_qty > 0:
            if lot_list: # Case: There's at least one full lot
                last_full_lot = lot_list[-1]
                # Default behavior: create new lot number
                ...
                if not is_update: # Ask user for preference
                    ...
                    if clicked == retain_btn:
                        ...
                        return None
                else: # When loading an update, just use the 'create new' logic
                    excess_lot_number = new_lot_num

            elif lot_input: # Case: No full lots, only excess
    ...
    def _populate_records_table(self, table, data, headers):
        ...
        if not data:
            # If there's no data, still stretch the columns to fill the space
            if header.count() > 0:
                header.setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
            return
        ...
        # Set resize modes for better layout
    ...
    def _populate_preview_table(self, table_widget, data, headers):
        ...
        # Ensure preview tables also resize properly
        table_widget.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def _update_dashboard_data(self):
        try:
            with self.engine.connect() as conn:
                endorsed_today = conn.execute(text(
                    "SELECT COUNT(*) FROM qce_endorsements_primary WHERE date_endorsed = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                qty_today = conn.execute(text(
                    "SELECT SUM(quantity_kg) FROM qce_endorsements_primary WHERE date_endorsed = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or Decimal(
                    '0.00')
                products_today = conn.execute(text(
                    "SELECT COUNT(DISTINCT product_code) FROM qce_endorsements_primary WHERE date_endorsed = CURRENT_DATE AND is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                lots_today = conn.execute(text(
                    "SELECT COUNT(*) FROM qce_endorsements_secondary s JOIN qce_endorsements_primary p ON s.system_ref_no = p.system_ref_no WHERE p.date_endorsed = CURRENT_DATE AND p.is_deleted IS NOT TRUE")).scalar_one_or_none() or 0
                recent_endorsements = conn.execute(text(
                    "SELECT system_ref_no, product_code, date_endorsed FROM qce_endorsements_primary WHERE is_deleted IS NOT TRUE ORDER BY id DESC LIMIT 5")).mappings().all()
                top_products = conn.execute(text(
                    "SELECT product_code, SUM(quantity_kg) as total_quantity FROM qce_endorsements_primary WHERE is_deleted IS NOT TRUE GROUP BY product_code ORDER BY total_quantity DESC LIMIT 5")).mappings().all()
            ...
            for row, record in enumerate(recent_endorsements):
                self.dashboard_recent_table.setItem(row, 0, QTableWidgetItem(record['system_ref_no']))
                ...
                self.dashboard_recent_table.setItem(row, 2, QTableWidgetItem(
                    record['date_endorsed'].strftime('%Y-%m-%d') if record[
                        'date_endorsed'] else ''))
            ...
            if not top_products:
                self.product_chart.setTitle("Top 5 Products by Total Endorsed Quantity (No Data)")
                return

            self.product_chart.setTitle("Top 5 Products by Total Endorsed Quantity")
            for prod in top_products:
                name = prod['product_code']
                total_quantity = float(prod.get('total_quantity') or 0.0)
