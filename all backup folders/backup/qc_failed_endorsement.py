...
# NEW MODULE - Based on the QC Failed Endorsement Sheet form (FM00120A).
# This module is adapted from fg_endorsement.py to provide similar functionality
# like lot range calculation, but with fields specific to this form.
# USER_FEATURES_V1 - Applied major feature parity with qc_excess_endorsement module.
# Includes: Deleted/Restore Tab, Deletion Password, and advanced lot breakdown logic.
# USER_FEATURES_V2 - Implemented intelligent auto-sizing for all tables to ensure they fill app width even when empty.
# USER_FEATURES_V3 - Decoupled record selection from viewing. Details now load only when the "View Details" tab is activated.
# USER_FEATURES_V4 - Refactored save logic. Excess quantity handling now occurs only during "Preview Breakdown".
# USER_FEATURES_V5 - Fixed bug where View Details was active on Deleted tab & added Refresh buttons.
...
class QCFailedEndorsementPage(QWidget):
    ...
    def _setup_dashboard_tab(self, tab):
        ...
        recent_group = QGroupBox("Recent QC Failed Endorsements")
    ...
    def _on_tab_changed(self, index):
        if index == self.tab_widget.indexOf(self.dashboard_tab):
            ...
            self._load_all_endorsements()
            self._on_record_selection_changed()  # Restore details tab state based on selection
        elif index == self.tab_widget.indexOf(self.entry_tab) and not self.current_editing_ref:
            self._load_combobox_data()
            self.tab_widget.setTabEnabled(self.tab_widget.indexOf(self.view_details_tab), False)
        elif index == self.tab_widget.indexOf(self.view_details_tab):
            self._show_selected_record_in_view_tab()
        elif index == self.tab_widget.indexOf(self.deleted_tab):
            self._load_deleted_records()
            # FIX: Explicitly disable details tab when on deleted tab
            self.tab_widget.setTabEnabled(self.tab_widget.indexOf(self.view_details_tab), False)
    ...
    def _save_endorsement(self):
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                ...
                QMessageBox.information(self, "Success", f"Endorsement {sys_ref_no} has been {action_text}.")
                self._clear_form()
                self._load_all_endorsements()
                self._update_dashboard_data()
    ...
    def _delete_record(self):
        ...
        reply = QMessageBox.question(self, "Confirm Deletion", f"Are you sure you want to delete endorsement {ref_no}?",
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self.log_audit_trail("DELETE_QC_FAILED", f"Soft-deleted endorsement: {ref_no}")
        ...
        try:
            with self.engine.connect() as conn:
                kpis = conn.execute(text("""
                    SELECT 
                        (SELECT COUNT(*) FROM qcf_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE) as endorsed_today,
                        (SELECT COALESCE(SUM(quantity_kg), 0) FROM qcf_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE) as qty_today,
                        (SELECT COUNT(DISTINCT product_code) FROM qcf_endorsements_primary WHERE endorsement_date = CURRENT_DATE AND is_deleted IS NOT TRUE) as products_today,
                        (SELECT COUNT(*) FROM qcf_endorsements_secondary s JOIN qcf_endorsements_primary p ON s.system_ref_no = p.system_ref_no WHERE p.endorsement_date = CURRENT_DATE AND p.is_deleted IS NOT TRUE) as lots_today
                """)).mappings().first()
                self.kpi_endorsed_value.setText(str(kpis['endorsed_today']))
                self.kpi_qty_value.setText(f"{kpis['qty_today'] or 0:.2f}")
                self.kpi_products_value.setText(str(kpis['products_today']))
                self.kpi_lots_value.setText(str(kpis['lots_today']))

                recent_data = conn.execute(text("""
                    SELECT system_ref_no, product_code, quantity_kg FROM qcf_endorsements_primary 
                    WHERE is_deleted IS NOT TRUE ORDER BY id DESC LIMIT 5
                """)).mappings().all()
                self._populate_records_table(self.dashboard_recent_table, recent_data,
                                             ["Ref No", "Product", "Qty (kg)"])

                top_prods = conn.execute(text("""
                    SELECT product_code, SUM(quantity_kg) as total_qty FROM qcf_endorsements_primary
                    WHERE is_deleted IS NOT TRUE AND quantity_kg > 0 GROUP BY product_code ORDER BY total_qty DESC LIMIT 5
                """)).mappings().all()
                self.product_pie_series.clear()
                if top_prods:
                    for prod in top_prods:
                        name = prod['product_code']
                        total_quantity = float(prod.get('total_qty') or 0.0)
                        slice_item = self.product_pie_series.append(f"{name}\n{total_quantity:.2f} kg", total_quantity)
                        slice_item.setLabelVisible(True)
        except Exception as e:
            QMessageBox.critical(self, "Dashboard Error", f"Error updating dashboard: {e}")