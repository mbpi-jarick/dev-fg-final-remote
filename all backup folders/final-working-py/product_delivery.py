...
class FloatLineEdit(QLineEdit):
    ...
    def _format_text(self):
        try:
            value = float(self.text() or 0.0); self.setText(f"{value:.2f}")
        except ValueError:
            self.setText("0.00")
...
class ProductDeliveryPage(QWidget):
    ...
    def _save_record(self):
        # ... (This method is unchanged from the corrected version) ...
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                ...
                if not dr_exists:
                    ...
                    return
                upsert_sql = text(
                    "INSERT INTO delivery_tracking (dr_no, status, scanned_by, scanned_on) VALUES (:dr_no, :status, :user, :now) ON CONFLICT (dr_no) DO UPDATE SET status = EXCLUDED.status, scanned_by = EXCLUDED.scanned_by, scanned_on = EXCLUDED.scanned_on;")
    ...
    def _print_receipt(self, dr_no):
        ...
        try:
            ...
            QMessageBox.critical(self, "PDF Generation Error", f"Could not generate PDF: {e}"); return
    ...
    def _generate_reportlab_pdf(self, primary_data, items_data):
        ...
        for item_row in items_data:
            ...
            if unit_price_val is not None and quantity_val is not None:
                try:
                    amount = float(unit_price_val) * float(quantity_val); amount_str = f"{amount:,.2f}"
                except (ValueError, TypeError):
                    amount_str = ""
            table_data.append(
