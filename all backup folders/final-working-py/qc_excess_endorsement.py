...
class QCExcessEndorsementPage(QWidget):
    ...
    def _handle_add_new_record(self, table_name, combo_to_update):
        # ... (This method is unchanged) ...
        title_map = {
            "qce_endorsers": "Endorser", "qce_receivers": "Receiver",
            "qce_bag_numbers": "Bag Number", "qce_box_numbers": "Box Number",
            "qce_remarks": "Remark"
        };
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, is_update=False):
        ...
        excess_lot_number = None

        # Determine the base for generating a new lot number
        ...
        if match:
            ...
            single_new_lot = f"{str(num_part + 1).zfill(num_len)}{suffix_part}"
        else:
            single_new_lot = f"{new_lot_base}-EXCESS"

        if is_range:  # Checkbox IS checked
            if total_qty <= 0 or weight_per_lot <= 0:
                QMessageBox.warning(self, "Input Error",
                                    "When generating a range, both Total Qty and Weight/Lot must be greater than 0.")
                return None

            num_full_lots = int(total_qty // weight_per_lot)
            excess_qty = total_qty % weight_per_lot

            # The new lots are generated based on the source lot number
            ...
            if excess_qty > 0:
                ...
                if match_last:
                    num_part_last, suffix_part_last, num_len_last = int(match_last.group(1)), match_last.group(2), len(
                        match_last.group(1))
                    excess_lot_number = f"{str(num_part_last + 1).zfill(num_len_last)}{suffix_part_last}"
                else:
                    excess_lot_number = f"{last_full_lot}-EXCESS"

        else:  # Checkbox is NOT checked
            # The entire quantity becomes a single new lot. Weight/Lot is ignored.
            lot_list = []
            excess_qty = total_qty
            excess_lot_number = single_new_lot
            weight_per_lot = Decimal(0)  # Not applicable in this mode

        return {"lots": lot_list, "excess_qty": excess_qty, "weight_per_lot": weight_per_lot,
                "excess_lot_number": excess_lot_number}

    def _save_record(self):
        # ... (This method is largely unchanged, just calls the updated transaction helper) ...
        required_fields = {
            "Form Ref No": self.form_ref_no_edit.text(),
            "Product Code": self.product_code_combo.currentText(),
            "Source Lot (Reference)": self.lot_number_edit.text(),
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                if is_update:
                    ...
                    conn.execute(text("DELETE FROM qce_endorsements_excess WHERE system_ref_no = :ref"),
                                 {"ref": sys_ref_no})
                    update_sql = text("""
                        UPDATE qce_endorsements_primary SET 
                        form_ref_no=:form_ref_no, product_code=:product_code, lot_number=:lot_number, 
                        quantity_kg=:quantity_kg, weight_per_lot=:weight_per_lot, status=:status, 
                        bag_number=:bag_number, box_number=:box_number, remarks=:remarks, 
                        date_endorsed=:date_endorsed, endorsed_by=:endorsed_by, date_received=:date_received, 
                        received_by=:received_by, edited_by=:edited_by, edited_on=:edited_on 
                    ...
                    insert_sql = text("""
                        INSERT INTO qce_endorsements_primary (
                            system_ref_no, form_ref_no, product_code, lot_number, quantity_kg, weight_per_lot, 
                            status, bag_number, box_number, remarks, date_endorsed, endorsed_by, date_received, 
                            received_by, encoded_by, encoded_on, edited_by, edited_on
                        ) VALUES (
                            :system_ref_no, :form_ref_no, :product_code, :lot_number, :quantity_kg, :weight_per_lot, 
                            :status, :bag_number, :box_number, :remarks, :date_endorsed, :endorsed_by, :date_received, 
                            :received_by, :encoded_by, :encoded_on, :edited_by, :edited_on
                ...
                breakdown_lots = self.preview_data.get('lots', [])
                if breakdown_lots:
                ...
                if self.preview_data.get('excess_qty', 0) > 0 and self.preview_data.get('excess_lot_number'):
            ...
            QMessageBox.information(self, "Success",
                                    f"QC Excess Endorsement {sys_ref_no} {action_text} and inventory transactions logged successfully.");
            self._clear_form();
    ...
    def _create_inventory_transactions(self, conn, primary_data, preview_data):
        ...
        # 1. Clear any previous transactions for this record
        ...
        # 2. Prepare all the new lots that will get a QTY_IN
        all_new_lots = []
        if preview_data.get('lots'):
            all_new_lots.extend(
                [{'lot_number': lot, 'quantity_kg': preview_data['weight_per_lot']} for lot in preview_data['lots']])
        if preview_data.get('excess_qty', 0) > 0 and preview_data.get('excess_lot_number'):
            all_new_lots.append(
                {'lot_number': preview_data['excess_lot_number'], 'quantity_kg': preview_data['excess_qty']})

        # 3. Create the QTY_IN transactions in the correct table based on status
        if primary_data["status"] == "Passed":
            transactions_in = []
            for lot_data in all_new_lots:
                transactions_in.append({
                    "transaction_date": primary_data["date_endorsed"], "transaction_type": "QC_EXCESS_IN_TO_FG",
                    "source_ref_no": sys_ref_no, "product_code": primary_data["product_code"],
                    "lot_number": lot_data["lot_number"], "quantity_in": lot_data["quantity_kg"], "quantity_out": 0,
                    "unit": "KG.", "warehouse": None, "encoded_by": self.username,
                    "remarks": f"Excess passed QC from source ref {primary_data['lot_number']}. QCE No: {sys_ref_no}"
                })
            if transactions_in:
            ...
            failed_transactions_in = []
            for lot_data in all_new_lots:
                failed_transactions_in.append({
                    "transaction_date": primary_data["date_endorsed"], "transaction_type": "QC_EXCESS_IN_TO_FAILED",
                    "source_ref_no": sys_ref_no, "product_code": primary_data["product_code"],
                    "lot_number": lot_data["lot_number"], "quantity_in": lot_data["quantity_kg"], "quantity_out": 0,
                    "unit": "KG.", "warehouse": None, "encoded_by": self.username,
                    "remarks": f"Excess failed QC from source ref {primary_data['lot_number']}. QCE No: {sys_ref_no}"
                })
            if failed_transactions_in:
    ...
    def _delete_record(self):
        # --- MODIFIED: DELETION LOGIC SIMPLIFIED ---
        selected_rows = self.records_table.selectionModel().selectedRows()
        if not selected_rows: return
        ref_no = self.records_table.item(selected_rows[0].row(), 0).text()
        ...
        if QMessageBox.question(self, "Confirm Delete",
            ...
            try:
                with self.engine.connect() as conn, conn.begin():
                    ...
                    # 2. Delete all inventory transactions created by this endorsement
                ...
                QMessageBox.information(self, "Success",
                                        f"Endorsement {ref_no} has been deleted and its inventory transactions have been removed.")
                self._refresh_all_data_views()
            except Exception as e:
    ...
    def _restore_record(self):
        # --- MODIFIED: RESTORE LOGIC SIMPLIFIED ---
    ...
    def _generate_preview_for_restore(self, primary_data, secondary_data, excess_data):
        ...
        excess_lot_number = None
        if excess_data:
            excess_qty = excess_data[0]['quantity_kg']
            excess_lot_number = excess_data[0]['lot_number']

        return {
            "lots": lots,
            "excess_qty": excess_qty,
            "weight_per_lot": primary_data['weight_per_lot'],
            "excess_lot_number": excess_lot_number
        }
    ...
    def _parse_lot_range(self, lot_input, num_lots):
        # ... (This method is unchanged) ...
        try:
            # For QCE, the lot_input is just a base. We need to create a range FROM it.
            start_str = lot_input
            start_match = re.match(r'^(\d+)([A-Z]*)$', start_str)
            if not start_match:
                raise ValueError(
                    "Source Lot format must be numeric with an optional text suffix (e.g., 12345 or 12345AA).")

            start_num, suffix, num_len = int(start_match.group(1)), start_match.group(2), len(start_match.group(1))

            # Generate the list of new lot numbers starting from the next number
