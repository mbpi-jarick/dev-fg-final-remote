...
class AddItemDialog(QDialog):
    ...
    def __init__(self, parent=None, data=None, warehouses=None, engine=None):
        ...
        # --- CHANGE: OK button is now always enabled ---
    ...
    def _reset_validation(self):
        """Resets the validation status when the user changes the lot number."""
        # --- CHANGE: No longer disables the OK button ---
    ...
    def _verify_lot_number(self):
        ...
        try:
            ...
            if result:
                ...
                # --- CHANGE: No longer enables the OK button (it's always enabled) ---
                # self.ok_button.setEnabled(True)
...
class ReceivingReportPage(QWidget):
    # ... (The rest of your ReceivingReportPage class, from __init__ onwards, is unchanged and complete) ...
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with self.engine.connect() as conn, conn.begin():
                    ...
                    # Fetch data needed for transaction logging
                    ...
                    if transactions_to_insert:
                        # First, ensure no old transactions exist for this (should have been deleted, but as a safeguard)
                        conn.execute(text(
                ...
                QMessageBox.information(self, "Success",
                                        f"Report {rr_no} has been restored and inventory transactions have been re-applied.")
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                if self.current_editing_rr_no:
                    ...
                    # If updating, first delete old transaction records to prevent duplicates
                    conn.execute(text(
                        """
                        DELETE FROM transactions 
                        WHERE source_ref_no = :rr_no AND transaction_type = 'RECEIVING_REPORT'
                        """
                    ), {"rr_no": rr_no})
                ...
                for item in items_data:
                    transactions_to_insert.append({
                        ...
                        "quantity_in": item["quantity_kg"],
                        "quantity_out": 0,
                        "unit": "KG.",  # Assuming KG, as it's the standard
                        "warehouse": item["location"],
                        "encoded_by": self.username,
                        "remarks": f"Received via RRRG No. {rr_no}"
                    })
            ...
            QMessageBox.information(self, "Success",
                                    f"Receiving Report {rr_no} {action_text} successfully and logged in transactions.");
            self._clear_form();
            self._refresh_all_data_views();
        except Exception as e:
    ...
    def _delete_record(self):
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                ...
                # --- NEW: Delete corresponding transaction records ---
                conn.execute(text(
                    """
                    DELETE FROM transactions 
                    WHERE source_ref_no = :rr_no AND transaction_type = 'RECEIVING_REPORT'
            ...
            QMessageBox.information(self, "Success",
                                    f"Report {rr_no} has been moved to the Deleted tab and inventory transactions have been reversed.")
            self._refresh_all_data_views()
        except Exception as e:
