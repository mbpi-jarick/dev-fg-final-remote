...
try:
    ...
    print("WARNING: qtawesome library not found. Buttons will not have icons.")
    fa = None
...
class RequisitionLogbookPage(QWidget):
    def __init__(self, db_engine, username, log_audit_trail_func):
        ...
        # --- MODIFIED: Pagination state for both active and deleted records ---
        self.current_page = 1
    ...
    # --- NEW: Method to set up the Deleted Records tab ---
    ...
    # --- MODIFIED: Handles loading data for the newly visible tab ---
    def _on_tab_changed(self, index):
        ...
        if tab_widget == self.view_details_tab:
            ...
            self._refresh_entry_combos()
        elif tab_widget == self.deleted_tab:  # --- NEW ---
    ...
    def _save_record(self):
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                ...
                # This is the "UPSERT" logic for the transaction table. It's safer and simpler than an UPDATE.
                ...
                # --- MODIFIED: This block handles the auto-refresh after saving ---
                self.tab_widget.setCurrentIndex(0)  # Switch to the main view tab
                self.search_edit.clear()  # Clearing the search triggers a full reload via the textChanged signal
                if not self.search_edit.text():  # If it was already empty, the signal won't fire, so force reload
                    self._load_all_records()
    ...
    # --- NEW: Helper function to manage transaction creation/updates to avoid code duplication ---
    def _update_or_create_transaction(self, conn, requisition_data):
        ...
        # 1. Delete any previous transaction associated with this requisition
        ...
        # 2. Prepare transaction data from the requisition data
        transaction_data = {
        ...
        # 3. Insert into the correct transaction table based on 'request_for' status
    ...
    # --- NEW: Method to load soft-deleted records ---
    ...
    def _update_deleted_pagination_controls(self):  # --- NEW ---
    ...
    def _delete_record(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with self.engine.connect() as conn, conn.begin():
                    # Soft-delete the requisition record
                    conn.execute(text(
                ...
                self._load_all_records()  # Refresh the main view
            except Exception as e:
                trace = traceback.format_exc();
    ...
    # --- NEW: Method to restore a soft-deleted record ---
    def _restore_record(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                self._load_deleted_records()  # Refresh the deleted records list
                self._load_all_records()  # Refresh the main list
            except Exception as e:
                trace = traceback.format_exc();
