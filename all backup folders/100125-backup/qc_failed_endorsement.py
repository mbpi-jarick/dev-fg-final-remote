...
# V9.0 - UI ENHANCEMENT: Removed icons to all buttons/tabs and styled the selected row color.
# V9.1 - FIX: Added a runnable __main__ block for standalone testing.
...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QTabWidget, QFormLayout, QLineEdit,
                             ...
                             QSplitter, QGridLayout, QGroupBox, QMenu, QDateTimeEdit, QStyle,
                             QApplication, QMainWindow)
from PyQt6.QtGui import QDoubleValidator # QIcon removed
...
class QCFailedEndorsementPage(QWidget):
    ...
    # --- UI SETUP METHODS ---
    def init_ui(self):
        # Set a global stylesheet for the widget to handle selected row color
        self.setStyleSheet("""
        ...
        # Add tabs with text (icons removed)
    ...
    def _setup_view_tab(self, tab):
        ...
        # style = self.style() # Removed since icons are removed
        ...
        # self.refresh_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_BrowserReload)) # Icon removed
        self.update_btn = QPushButton("Load Selected for Update")
        # self.update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp)) # Icon removed
        self.update_btn.setObjectName("update_btn")
        self.delete_btn = QPushButton("Delete Selected")
        # self.delete_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon)) # Icon removed
        ...
        # self.prev_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowBack)) # Icon removed
        self.next_btn = QPushButton("Next ")
        # self.next_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowForward)) # Icon removed
    ...
    def _setup_deleted_tab(self, tab):
        ...
        # self.restore_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogResetButton)) # Icon removed
        self.restore_btn.setObjectName("update_btn")
        self.restore_btn.setEnabled(False)

        self.refresh_deleted_btn = QPushButton("Refresh")
        # self.refresh_deleted_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_BrowserReload)) # Icon removed
    ...
    def _setup_entry_tab(self, tab):
        ...
        # self.preview_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileLinkIcon)) # Icon removed
        self.save_btn = QPushButton("Save Endorsement")
        # self.save_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton)) # Icon removed
        self.clear_btn = QPushButton("New")
        # self.clear_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileIcon)) # Icon removed
        self.cancel_update_btn = QPushButton("Cancel Update")
        # self.cancel_update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton)) # Icon removed
    ...
    def _create_managed_combo(self, table_name, combo):
        ...
        # manage_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView)) # Icon removed
    ...
    def _show_records_table_context_menu(self, pos):
        ...
        # view_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_MessageBoxInformation)) # Icon removed
        edit_action = menu.addAction("Load for Update")
        # edit_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp)) # Icon removed
        delete_action = menu.addAction("Delete Record")
        # delete_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon)) # Icon removed
    ...
    def _show_deleted_table_context_menu(self, pos):
        ...
        # restore_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogResetButton)) # Icon removed
        action = menu.exec(self.deleted_records_table.mapToGlobal(pos))
        if action == restore_action:
            self._restore_record()

    # --- ALL OTHER METHODS ARE UNCHANGED ---
    # The core logic for saving, deleting, restoring, and calculating endorsements
    # remains identical to your working V8.1 version.
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                if is_update:
                    ...
                    insert_sql = text("""INSERT INTO qcf_endorsements_primary (id, system_ref_no, form_ref_no, endorsement_date, 
                        ...
                        VALUES (NULL, :system_ref_no, :form_ref_no, :endorsement_date, :product_code, :lot_number, 
    ...
    def _load_all_endorsements(self):
        ...
        try:
            ...
            self._populate_records_table(self.records_table, [], ["Sys Ref", "Form Ref", "Date", "Product Code", "Lot Input", "Qty (kg)"])
    ...
    def _load_deleted_records(self):
        try:
            ...
            self._populate_records_table(self.deleted_records_table, [], ["Sys Ref", "Form Ref", "Date", "Product", "Deleted By", "Deleted On"])
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            if record.get('endorsement_date'): self.endorsement_date_edit.setDate(QDate.fromString(str(record['endorsement_date']), "yyyy-MM-dd"))
...
if __name__ == "__main__":
    ...
    try:
        with mock_engine.connect() as conn, conn.begin():
            conn.execute(text("CREATE TABLE qcf_endorsements_primary (id INTEGER PRIMARY KEY, system_ref_no TEXT UNIQUE, form_ref_no TEXT, endorsement_date TEXT, product_code TEXT, lot_number TEXT, quantity_kg REAL, weight_per_lot REAL, endorsed_by TEXT, warehouse TEXT, received_by_name TEXT, received_date_time TEXT, encoded_by TEXT, encoded_on TEXT, edited_by TEXT, edited_on TEXT, is_deleted BOOLEAN DEFAULT FALSE);"))
            conn.execute(text("CREATE TABLE qcf_endorsements_secondary (system_ref_no TEXT, lot_number TEXT, quantity_kg REAL);"))
            conn.execute(text("CREATE TABLE qcf_endorsements_excess (system_ref_no TEXT, lot_number TEXT, quantity_kg REAL);"))
            ...
            conn.execute(text("CREATE TABLE failed_transactions(id INTEGER PRIMARY KEY, transaction_date TEXT, transaction_type TEXT, source_ref_no TEXT, product_code TEXT, lot_number TEXT, quantity_in REAL, quantity_out REAL, unit TEXT, warehouse TEXT, encoded_by TEXT, remarks TEXT);"))
            conn.execute(text("CREATE TABLE transactions(id INTEGER PRIMARY KEY, transaction_date TEXT, transaction_type TEXT, source_ref_no TEXT, product_code TEXT, lot_number TEXT, quantity_in REAL, quantity_out REAL, unit TEXT, warehouse TEXT, encoded_by TEXT, remarks TEXT);"))
