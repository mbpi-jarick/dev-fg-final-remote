...
# --- Custom UpperCaseLineEdit Widget (Reused) ---
...
class FailedInventoryWorker(QObject):
    """
    Calculates the hypothetical inventory balance based on
    the specific failed beginning inventory table (beg_invfailed1) and failed_transactions data.
    """
        ...
        try:
            ...
            query_str = f"""
                WITH combined_movements AS (
                    ...
                    FROM beg_invfailed1 -- <--- THIS LINE IS UPDATED
...
# --- Main Page Class: Failed Inventory Report ---
class FailedInventoryReportPage(QWidget):
    ...
    def _get_styles(self):
        # Using a slightly different color to distinguish Failed vs Good inventory
        return """
            QTableWidget { outline: 0; } 
            QTableWidget::item:selected {
                background-color: #d9534f; /* Reddish selection for 'Failed' */
                color: white;
            }
            #TotalBalanceLabel { color: #D9534F; } /* Red text */
    ...
    def init_ui(self):
        ...
        # Label updated to reflect failed records
        self.total_balance_label = QLabel("Failed Record Balance: 0.00 kg", objectName="TotalBalanceLabel")
    ...
    def _create_filter_controls(self):
        # --- FILTER GROUPBOX ---
        controls_group = QGroupBox("Filters & Actions (Failed Inventory Summary)")
        ...
        self.lot_number_input = UpperCaseLineEdit(placeholderText="Filter by Lot number in Failed Records...")
    ...
    def _start_inventory_calculation(self):
        ...
        # Use the specific worker for failed transactions
    ...
    def _display_inventory(self, df, is_specific_lot):
        ...
        label_text = (
            f"Failed Lot Balance: {total_balance:.2f} kg"
            if is_specific_lot and len(df) > 0
            else f"Overall Failed Record Balance: {total_balance:.2f} kg"
        )
