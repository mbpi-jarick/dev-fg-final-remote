...
class ProductDeliveryPage(QWidget):
    ...
    def init_ui(self):
        ...
        # Removed all QStyle.StandardPixmap icon settings for tabs
    ...
    def _toggle_camera(self):
        ...
        if hasattr(self, 'camera_thread') and self.camera_thread and self.camera_thread.isRunning():
            ...
            # Removed Icon: self.toggle_camera_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaStop))
    ...
    def _on_camera_thread_finished(self):
        ...
        # Removed Icon: self.toggle_camera_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaPlay))
    ...
    def _handle_qr_code(self, data: str):
        ...
        if self.scanner_input.text() != data:
            ...
            # --- QR Code Detection ---
            # NOTE: This block was copied incorrectly into the background thread in the previous version
            # If the user provided the corrected method block, I will ensure it looks right here.
            # Assuming the previous implementation was only emitting the data and didn't have redundant decoding here:
            # (No redundant decoding needed here, as the signal already carries the data)
    ...
    def _setup_tracking_tab(self, tab):
        ...
        # Removed Icon: refresh_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_BrowserReload))
    ...
    def _setup_scanner_tab(self, tab):
        ...
        # Removed Icon: scan_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogOkButton))
        ...
        # Removed Icon: clear_log_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
    ...
    def _setup_lot_breakdown_tab(self, tab):
        ...
        # style = self.style() # Keep style definition if used elsewhere, but not needed here now.
        self.breakdown_save_btn = QPushButton("Save Breakdown");
        # Removed Icon: self.breakdown_save_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton))
        preview_btn = QPushButton("Preview Breakdown")
        # Removed Icon: preview_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))
        clear_btn = QPushButton("Clear");
        # Removed Icon: clear_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogResetButton))
    ...
    def _setup_view_tab(self, tab):
        ...
        style = self.style()
        refresh_btn = QPushButton("Refresh")
        # Removed Icon: refresh_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_BrowserReload))
        top_layout.addWidget(refresh_btn)
        self.update_btn = QPushButton("Load Selected for Update")
        # Removed Icon: self.update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp))
        self.delete_btn = QPushButton("Delete Selected")
        # Removed Icon: self.delete_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
        ...
        # Removed Icon: self.prev_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowBack))
        self.next_btn = QPushButton("Next ")
        # Removed Icon: self.next_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowForward))
    ...
    def _setup_view_details_tab(self, tab):
        # ... (This method is unchanged as it contained no icons) ...
    ...
    def _setup_entry_tab(self, tab):
        # FIX: Removed the redundant 'layout = QVBoxLayout(tab)' line to prevent UI conflict.
        # layout = QVBoxLayout(tab)
        main_layout = QVBoxLayout(tab)
        ...
        # Removed Icon: add_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogApplyButton))
        # Removed Icon: self.edit_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogInfoView))
        self.remove_item_btn = QPushButton("Remove Item")
        # Removed Icon: self.remove_item_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton))
        ...
        # Removed Icon: self.cancel_update_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogCancelButton))
        # Removed Icon: self.clear_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileIcon))
        self.save_btn, self.print_btn = QPushButton("Save"), QPushButton("Print Preview")
        # Removed Icon: self.save_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton))
        # Removed Icon: self.print_btn.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))
    ...
    # --- Standard methods below (kept unchanged except for icon removals in context menus) ---
    ...
    def _show_records_table_context_menu(self, pos):
        ...
        # Removed Icon: view_dr_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_FileDialogContentsView))

        view_details_action = menu.addAction("View Record Details");
        # Removed Icon: view_details_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_MessageBoxInformation))
        menu.addSeparator()
        edit_action = menu.addAction("Edit Record");
        # Removed Icon: edit_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_ArrowUp))
        delete_action = menu.addAction("Delete Record");
        # Removed Icon: delete_action.setIcon(style.standardIcon(QStyle.StandardPixmap.SP_TrashIcon))
    ...
    def _show_deleted_table_context_menu(self, pos):
        ...
        # Removed Icon: restore_action.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_DialogResetButton))
