...
import sys
import numpy as np
import traceback  # Added explicit traceback import
from datetime import datetime, timedelta
from collections import defaultdict
import qtawesome as fa

# PyQt6 Imports
from PyQt6.QtCore import Qt, QObject, pyqtSignal, QThread, QDate, QSize, QUrl
...
# --- WORKER: For Calculating Inventory Summary (Current Balance Logic) ---
class InventoryWorker(QObject):
    ...
    def run(self):
        """
        Calculates the current balance for each lot by combining Beginning Inventory
        and Transactions (up to the as_of_date).
        """
        try:
            params = {}
            date_filter_clause = ""
            ...
            if self.as_of_date:
                # The date filter only applies to the dynamic 'transactions' table
            ...
            # 1. Combined Movements CTE: Sums up beginning inventory and transactions.
            combined_movements_query = f"""
                (
                    -- Beginning Inventory (treated as 100% IN)
                    SELECT
                    ...
                    SELECT
                        UPPER(TRIM(product_code)) AS product_code, UPPER(TRIM(lot_number)) AS lot_number,
                        COALESCE(CAST(quantity_in AS NUMERIC), 0), COALESCE(CAST(quantity_out AS NUMERIC), 0)
                    FROM transactions
                    WHERE product_code IS NOT NULL AND TRIM(product_code) <> '' 
                      AND lot_number IS NOT NULL AND TRIM(lot_number) <> '' 
                      {date_filter_clause} -- Apply date filter only to transactions
                )
            """
            ...
            # 2. Final Aggregation Query: Calculates the net balance per lot
            query_str = f"""
                ...
                GROUP BY lot_number
                -- Only show lots with a positive balance (FG Passed)
            ...
            # Use imported traceback for detailed debugging output
            traceback.print_exc()


# --- ADVANCED DASHBOARD WIDGET (Maximized Space Fixes Applied) ---
class DashboardWidget(QWidget):
    def __init__(self):
        ...
        summary_group.setSizePolicy(QSizePolicy.Policy.Expanding,
        ...
        contribution_group.setSizePolicy(QSizePolicy.Policy.Expanding,
        ...
        self.lot_stats_group.setSizePolicy(QSizePolicy.Policy.Expanding,
                                           QSizePolicy.Policy.Expanding)
...
class GoodInventoryPage(QWidget):
    ...
    def _get_styles(self) -> str:
        ...
        return f"""
            ...
            /* Override label background in transparent containers like HeaderWidget */
            #HeaderWidget QLabel {{
    ...
    def init_ui(self):
        ...
        icon_pixmap = fa.icon('fa5s.calculator', color="#3a506b").pixmap(QSize(28, 28))
        icon_label = QLabel()
        ...
        # Tab Setup
        self.tab_widget.addTab(self.inventory_tab, fa.icon('fa5.list-alt', color=PRIMARY_ACCENT_COLOR),
                               "Inventory Details")
        self.tab_widget.addTab(self.dashboard_widget, fa.icon('fa5s.chart-area', color=PRIMARY_ACCENT_COLOR),
                               "Dashboard")
    ...
    def _create_filter_controls(self) -> QGroupBox:
        ...
        self.refresh_button.setIcon(fa.icon('fa5s.sync-alt', color=PRIMARY_ACCENT_COLOR))
        filter_layout.addWidget(self.refresh_button)

        self.export_button = QPushButton("Export", objectName="PrimaryButton")
        self.export_button.setIcon(fa.icon('fa5s.file-excel', color=PRIMARY_ACCENT_COLOR))
    ...
    def _start_inventory_calculation(self):
        ...
        # Initialize worker with filters and engine
    ...
    def set_controls_enabled(self, enabled: bool):
        ...
        # Export button is only enabled if controls are enabled AND data exists
...
def setup_postgres_engine() -> Engine:
    ...
    # NOTE: Placeholder DB configuration. Replace with actual working credentials.
    DB_CONFIG = {
        "host": "192.168.1.13",
        "port": 5432,
        "database": "dbfg",
    ...
    try:
        ...
        # Attempt connection immediately to verify credentials
        with engine.connect():
            pass
        return engine
    except Exception as e:
        ...
        msg = QMessageBox()
        msg.setIconPixmap(fa.icon('fa5s.database', color='#dc3545').pixmap(32, 32))
        msg.setWindowTitle("Database Connection Error")
        msg.setText(f"Failed to connect to PostgreSQL at {DB_CONFIG['host']}:{DB_CONFIG['port']}.")
