...
class FloatLineEdit(QLineEdit):
    ...
    def _format_text(self):
        try:
            ...
            # Apply comma formatting using the helper
...
class FGEndorsementPage(QWidget):
    ...
    def _check_inventory_status(self):
        ...
        try:
            ...
            if found_record:
                ...
                # --- MODIFIED: Use comma formatting here ---
                status_text = f"Status: Found | Product: {pcode} | Available Qty: {format_float_with_commas(qty_decimal)} kg"
    ...
    def _update_calculated_lots_display(self):
        try:
            total_qty = Decimal(self.quantity_edit.value())
            weight_per_lot = Decimal(self.weight_per_lot_edit.value())

            if weight_per_lot > 0:
                num_lots = int(total_qty // weight_per_lot)
                excess = total_qty % weight_per_lot
                # --- MODIFIED: Use comma formatting here ---
                self.calculated_lots_label.setText(
    ...
    def _validate_and_calculate_lots(self):
        try:
            ...
            if self.is_lot_range_check.isChecked() and (Decimal(str(total_qty)) % Decimal(str(weight_per_lot)) > 0):
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, excess_handling_method):
        total_qty, weight_per_lot = Decimal(str(total_qty)), Decimal(str(weight_per_lot))
        ...
        if excess_qty > 0:
            if is_range and excess_handling_method == 'ASSIGN_TO_LAST_IN_RANGE':
                # This case should merge with the last lot in breakdown_data, not be separate
                if breakdown_data:
                    breakdown_data[-1]['quantity_kg'] += excess_qty
                else:  # Should not happen if there are lots, but as a fallback
    ...
    def _populate_preview_widgets(self, data):
        ...
        total_breakdown = sum(Decimal(str(item['quantity_kg'])) for item in breakdown_data)
        self.breakdown_total_label.setText(f"<b>Total: {format_float_with_commas(total_breakdown)} kg</b>")

        self._populate_preview_table(self.preview_excess_table, excess_data, ["Excess Lot", "Excess Qty (kg)"])
        total_excess = sum(Decimal(str(item['quantity_kg'])) for item in excess_data)
        self.excess_total_label.setText(f"<b>Total: {format_float_with_commas(total_excess)} kg</b>")
    ...
    def _populate_view_details_tab(self, sys_ref_no):
        try:
            ...
            total_breakdown = sum(item.get('quantity_kg', Decimal('0.0')) for item in breakdown)
            self.view_breakdown_total_label.setText(f"<b>Total: {format_float_with_commas(total_breakdown)} kg</b>")

            self._populate_view_table(self.view_excess_table, excess, ["Excess Lot", "Excess Qty (kg)"])
            total_excess = sum(item.get('quantity_kg', Decimal('0.0')) for item in excess)
    ...
    def _populate_view_table(self, table_widget: QTableWidget, data: list, headers: list):
        ...
        for i, row_data in enumerate(data):
            for j, key in enumerate(keys):
                ...
                item_text = format_float_with_commas(val) if isinstance(val, (Decimal, float)) else str(
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            # Use the new value() method which handles comma removal
    ...
    # --- MODIFIED: Generalized table population logic ---
    def _populate_records_table(self, table: QTableWidget, data: list, headers: list):
        ...
        for i, row_data in enumerate(data):
            for j, key in enumerate(keys):
                ...
                # Format any numeric type with commas
                if isinstance(val, (Decimal, float)):
                    item_text = format_float_with_commas(val)
                else:
