...
import qtawesome as fa  # Use 'fa' alias as per the original code
...
class DashboardWidget(QWidget):
    def __init__(self):
        ...
        summary_group.setSizePolicy(QSizePolicy.Policy.Expanding,
                                    QSizePolicy.Policy.Minimum)  # Use minimum vertical space
        ...
        contribution_group.setSizePolicy(QSizePolicy.Policy.Expanding,
                                         QSizePolicy.Policy.Expanding)  # Expand vertically
        ...
        # --- FIX: Ensure the second row (containing the tables) stretches ---
        grid_layout.setRowStretch(1, 1)
        # -------------------------------------------------------------------

        self.layout.addLayout(grid_layout)
        # FIX: Removed self.layout.addStretch(1)
    ...
    def _create_contribution_table(self) -> QTableWidget:
        ...
        # Removed setMaximumHeight(350) to allow stretch
        return table
    ...
    def _create_lot_statistics_group(self) -> QGroupBox:
        ...
        # Added row stretch here to ensure the statistics fill the vertical space provided by the GroupBox
...
class GoodInventoryPage(QWidget):
    ...
    def init_ui(self):
        ...
        # This widget is crucial for maximizing space, allowing the QTabWidget to grow
...
def setup_postgres_engine() -> Engine:
    ...
    try:
        ...
        # Use QUrl for icon path if running standalone
        icon_path = fa.icon('fa5s.database', color='#dc3545').pixmap(32, 32).toImage()
...
if __name__ == "__main__":
    ...
    # --- SETUP VIRTUAL TABLES FOR STANDALONE TEST ---
    # Since the real DB might not be available, we mock the engine and tables
    # if you are running this locally without a PostgreSQL instance set up.

    try:
        db_engine = setup_postgres_engine()
    except SystemExit:
        # Exit handled by setup_postgres_engine if connection fails
        sys.exit(1)
