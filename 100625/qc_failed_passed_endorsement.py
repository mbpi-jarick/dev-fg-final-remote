# qc_failed_passed_endorsement.py
# FINAL VERSION - Rebuilt with a modern UI, pagination, and features consistent with other modules.
# FIX - Added missing pagination logic, corrected tab switching, and fixed loading records for update.
# UPDATE - Added QC ID lookup from 'dbmbpi' database to populate product code and lot number.
# UPDATE - Implemented the QC ID lookup as a persistent, searchable combo box directly on the entry form.
# UPDATE - Moved secondary database credentials to a DB_CONFIG dictionary for better maintainability.
# V2 - Added Deleted Tab with Restore, and standardized table UI (borderless, no focus, row selection).
# V3 - Upgraded lot range and preview logic to match fg_endorsement.py for superior handling of excess quantities.
# V3.1 - Fixed NameError on DB_CONFIG and added traceback for robust error logging.
# V3.2 - Corrected excess handling logic to prompt for both single lots and ranges.
# V4 - Removed Dashboard for a streamlined, function-focused UI.
# V5 (USER REQUEST) - Integrated with failed_transactions (QTY_OUT) and transactions (QTY_IN).
...
# --- UI UNIFICATION UPDATE (USER REQUEST) ---
# MODIFIED: UI has been updated to match the style of qc_failed_endorsement.py.
# This includes a left-aligned header with an icon, styled buttons (Primary, Update, Delete),
# instructional banners on each tab, and themed tab icons.
...
# --- Configuration ---
ICON_COLOR = '#27ae60'  # Green theme for "Passed" module
...
class QCFailedPassedPage(QWidget):
    ...
    def init_ui(self):
        ...
        icon_label = QLabel()
        if fa:
            icon_pixmap = fa.icon('fa5s.check-circle', color=ICON_COLOR).pixmap(QSize(28, 28))
    ...
    def _setup_view_tab(self, tab):
        ...
        if fa:
            self.refresh_records_btn.setIcon(fa.icon('fa5s.sync-alt', color=ICON_COLOR))
        ...
        # Moved configuration to a central method
    ...
    def _setup_deleted_tab(self, tab):
        ...
        if fa:
            self.deleted_refresh_btn.setIcon(fa.icon('fa5s.sync-alt', color=ICON_COLOR))
    ...
    def _setup_entry_tab(self, tab):
        ...
        if fa:
            ...
            self.clear_btn.setIcon(fa.icon('fa5s.eraser', color=ICON_COLOR))
    ...
    def _create_combo_with_add(self, table_name, combo):
        ...
        if fa:
            add_btn.setIcon(fa.icon('fa5s.plus-circle', color=ICON_COLOR))
