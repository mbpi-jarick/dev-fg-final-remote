...
# --- Database Imports (Using SQLite in memory for standalone testing) ---
from sqlalchemy import create_engine, text, inspect


# --- DUMMY DEPENDENCIES ---

# Mock function for logging audit trails
...
def setup_in_memory_db():
    ...
    with engine.connect() as conn:
        ...
        # 3. Product Code/Production (Dummy data for demo)
        conn.execute(text("CREATE TABLE IF NOT EXISTS legacy_production (prod_code TEXT UNIQUE)"))

        # 4. Transaction tables (must exist for save logic to run)
        conn.execute(text("""
        ...
        # Insert initial lookup data (uses ON CONFLICT DO NOTHING, which is fine)
...
# --- HELPER CLASSES (Unchanged from original structure) ---
...
class RequisitionLogbookPage(QWidget):
    # Define primary color for header/icon theme
    ...
    def _create_instruction_box(self, text):
        ...
        icon_label = QLabel()
        # Using valid FontAwesome icon name
        ...
        # Custom style for instruction box
    ...
    def init_ui(self):
        ...
        # --- MODIFIED: Structured Header (Icon, Title) ---
        header_widget = QWidget(objectName="HeaderWidget")
        header_layout = QHBoxLayout(header_widget)
        header_layout.setContentsMargins(0, 0, 0, 10)  # Added margin bottom
        ...
        # --- Tab Setup (ICONS CORRECTED with valid FontAwesome names) ---
        ...
        # --- MODIFIED: Added stylesheet to set the selected row color for all tables and header styles ---
        stylesheet = f"""
            QTableWidget::item:selected {{
                background-color: {self.PRIMARY_ACCENT_COLOR};
                color: white;
            }}
            /* Specific styles for the newly created header */
            QLabel#PageHeader {{ 
                font-size: 24px; 
                font-weight: bold; 
                color: {"#3a506b"};
    ...
    def _setup_view_tab(self, tab):
        ...
        instructions = self._create_instruction_box(
            "Use this tab to view, search, load, or delete active requisitions. Double-click a row or use 'Load Selected for Update' to edit the record.")
        ...
        # --- Action buttons (ICONS CORRECTED) ---
        ...
        # --- Pagination buttons (ICONS CORRECTED) ---
    ...
    def _setup_entry_tab(self, tab):
        ...
        # --- Instructions ---
        instructions = self._create_instruction_box(
            "Enter new requisition details. If updating, the Requisition ID field will be populated. Ensure all required fields (Product Code) are filled.")
        layout.addWidget(instructions)
        ...
        # --- Entry buttons (ICONS CORRECTED) ---
        self.save_btn = QPushButton(fa.icon('fa5s.save'), "Save Requisition")
        self.save_btn.setObjectName("PrimaryButton")
        self.clear_btn = QPushButton(fa.icon('fa5s.file'), "New")
        self.clear_btn.setObjectName("SecondaryButton")
        self.cancel_update_btn = QPushButton(fa.icon('fa5s.times-circle'), "Cancel Update")
        ...
        if dialog.exec():
            ...
            try:
                with self.engine.connect() as conn, conn.begin():
                    # This section already uses the correct ON CONFLICT syntax
                    sql = text(
    ...
    def _setup_view_details_tab(self, tab):
        ...
        # --- Instructions ---
        instructions = self._create_instruction_box(
            "This read-only tab displays all metadata for the currently selected requisition record from the 'All Requisitions' tab.")
        layout.addWidget(instructions)
    ...
    def _clear_form(self):
        ...
        self.save_btn.setIcon(fa.icon('fa5s.save'))  # Reset icon (CORRECTED)
    ...
    def _generate_req_id(self):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # Check if last_ref is not None before splitting
                if last_ref:
                    # Safely handle potential error if the suffix is non-numeric
                    try:
                        suffix = int(last_ref.split('-')[-1])
                        return f"{prefix}{suffix + 1:04d}"
                    except ValueError:
                        # If parsing fails, start fresh or handle error appropriately
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                for value, table, column in [
                    ...
                    if value:
                        # FIX APPLIED HERE: Use standard INSERT ... ON CONFLICT DO NOTHING
                        sql_upsert = text(
                            f"INSERT INTO {table} ({column}) VALUES (:value) ON CONFLICT ({column}) DO NOTHING"
                        )
                        conn.execute(sql_upsert, {"value": value})
                ...
                if is_update:
                    sql = text("""UPDATE requisition_logbook SET
                                  ...
                                  edited_by=:user, edited_on=:timestamp WHERE req_id=:req_id""")
                    ...
                    sql = text("""INSERT INTO requisition_logbook (req_id, manual_ref_no, category, request_date, requester_name, 
                                  ...
                                  :quantity_kg, :status, :approved_by, :remarks, :location, :request_for, :user, :timestamp, :user, :timestamp)""")
                    action, log_action = "saved", "CREATE_REQUISITION"

                # SQLite timestamp handling (using ISO format string for consistency)
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            if req_date:
                # Handle conversion from string (SQLite) or date object
            ...
            # Update button state (Icon CORRECTED)
    ...
    def _load_all_records(self):
        ...
        try:
            ...
            # In standalone mode, showing the error is helpful
    ...
    def _populate_records_table(self, table, headers, data):
        ...
        # Mapping for accessing dictionary results (handling SQL mapping keys)
        key_map = {
            "req_id": "req_id", "manual_ref_no": "manual_ref_no", "date": "request_date",
            "product_code": "product_code", "lot_no": "lot_no", "qty (kg)": "quantity_kg",
            "status": "status", "location": "location", "for": "request_for",
            "last edited by": "edited_by", "last edited on": "edited_on",
            "deleted by": "deleted_by", "deleted on": "deleted_on"
        }

        keys_in_order = [key_map.get(h.lower(), h.lower()) for h in headers]
        ...
        for row_idx, record in enumerate(data):
            for col_idx, key in enumerate(keys_in_order):
                ...
                elif isinstance(value, str) and ('date' in key or 'on' in key) and len(value) > 10 and value[10] == ' ':
                    # Attempt to format SQLite timestamp strings
                    try:
                        dt_obj = datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
                        text_val = dt_obj.strftime('%Y-%m-%d %H:%M')
                    except ValueError:
                        text_val = value
                else:
    ...
    def _show_selected_record_in_view_tab(self):
        ...
        try:
            ...
            for key, value in record.items():
                ...
                elif isinstance(value, str) and len(value) > 10 and value[10] == ' ':  # SQLite timestamp string check
                    try:
                        dt_obj = datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
                        display_value = dt_obj.strftime('%Y-%m-%d %H:%M')
                    except ValueError:
    ...
    def _delete_record(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with self.engine.connect() as conn, conn.begin():
                    # SQLite uses 0/1 for boolean, edited_on needs to be set
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    conn.execute(text(
                        "UPDATE requisition_logbook SET is_deleted = 1, edited_by = :user, edited_on = :timestamp WHERE req_id = :req_id"),
                        {"req_id": req_id, "user": self.username, "timestamp": timestamp})
                    conn.execute(text("DELETE FROM transactions WHERE source_ref_no = :req_id"), {"req_id": req_id})
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with self.engine.connect() as conn, conn.begin():
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    conn.execute(text(
                        "UPDATE requisition_logbook SET is_deleted = 0, edited_by = :user, edited_on = :timestamp WHERE req_id = :req_id"),
                        {"req_id": req_id, "user": self.username, "timestamp": timestamp})
...
class StandaloneWindow(QMainWindow):
    def __init__(self, db_engine):
        ...
        self.page = RequisitionLogbookPage(
            db_engine=db_engine,
        ...
        # Apply a minimal global style for appearance, including new header styles
        self.setStyleSheet("""
            QWidget { font-size: 10pt; }
            #PrimaryButton { background-color: #007BFF; color: white; border-radius: 4px; padding: 5px; }
            #PrimaryButton:hover { background-color: #0056b3; }
            #SecondaryButton { background-color: #f8f9fa; color: #333; border: 1px solid #ccc; border-radius: 4px; padding: 5px; }
            #SecondaryButton:hover { background-color: #e2e6ea; }
            ...
            QTabBar::tab { background: #f0f0f0; padding: 8px 15px; border: 1px solid #ccc; border-bottom: none; }
            QTabBar::tab:selected { background: white; border-color: #ccc; }

            /* Styles for the new structured header */
            QLabel#PageHeader { 
                font-size: 24px; 
                font-weight: bold; 
                color: #007BFF;
...
if __name__ == "__main__":
    # Check if QApplication exists, otherwise create one (important for external IDE execution)
