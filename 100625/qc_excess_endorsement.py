...
def create_db_engine():
    ...
    # Using a memory SQLite database for mock standalone run if Postgres fails
    try:
        db_url = (f"postgresql://{DB_CONFIG['user']}:{DB_CONFIG['password']}"
                  f"@{DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['dbname']}")
        engine = create_engine(db_url, pool_pre_ping=True, pool_recycle=3600)
        # Attempt a connection to check validity
        with engine.connect():
        ...
        # Fallback to SQLite (requires database schema setup below)
...
class QCExcessEndorsementPage(QWidget):
    ...
    def init_ui(self):
        ...
        # --- MODIFIED HEADER START (Adding Icon) ---
    ...
    def _setup_view_tab(self, tab):
        ...
        self.records_table.doubleClicked.connect(self._load_record_for_update)  # Enable double-click to edit
        layout.addWidget(self.records_table)

        pagination_layout = QHBoxLayout()
        # Icon setup for pagination buttons
    ...
    def _show_records_table_context_menu(self, position):
        ...
        menu = QMenu()
        # Adding icons to context menu actions
    ...
    def _show_deleted_table_context_menu(self, position):
        ...
        menu = QMenu()
        # Adding icon to context menu action
    ...
    def _load_combobox_data(self):
        ...
        try:
            ...
            # Handle case where the mock tables might not exist if using a real connection that failed.
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, is_update=False):
        ...
        if is_range:
            ...
            if excess_qty > 0:
                # V4 FIX: Find the last lot generated and create the excess lot *after* it, instead of after the input lot.
            ...
            # If not generating a range, the entire quantity becomes excess and gets a single new lot number.
    ...
    def _load_all_records(self):
        ...
        try:
            ...
            # Only show DB critical error if it's not the initial mock DB setup failing quietly
            if "no such table" not in str(e).lower():
    ...
    def _parse_lot_range(self, lot_input, num_lots):
        try:
            ...
            # Start generating lot numbers *after* the input lot number (start_num + 1)
...
if __name__ == '__main__':
    ...
    # Initialize DB Engine (will use mock SQLite if connection fails)
    db_engine = create_db_engine()

    main_window = QCExcessEndorsementPage(db_engine, USERNAME, log_audit_trail_func)

    # Set window style for better visual appearance (basic styling)
