...
class FGEndorsementPage(QWidget):
    ...
    def init_ui(self):
        ...
        icon_label = QLabel()
        icon_label.setPixmap(fa.icon('fa5s.file-signature', color='#3a506b').pixmap(32, 32))
        header_layout.addWidget(icon_label)

        title_label = QLabel("Finished Goods Endorsement")
    ...
    def _setup_entry_tab(self, tab):
        ...
        # Modified clear button connection to ensure it calls full reset
        self.clear_btn.clicked.connect(lambda: self._clear_form(post_save_data=None, is_manual_clear=True))
        self.cancel_update_btn.clicked.connect(lambda: self._clear_form(post_save_data=None, is_manual_clear=True))
        ...
        # Initial call to clear form (full clear)
        self._clear_form(post_save_data=None, is_manual_clear=False)
    ...
    def _save_endorsement(self):
        ...
        try:
            ...
            # --- POST-SAVE FORM HANDLING ---

            message = f"Endorsement {sys_ref_no} {'updated' if is_update else 'saved'} successfully."
            self.show_notification(message, 'success')

            if is_update:
                # Full clear after update
                self._clear_form(post_save_data=None, is_manual_clear=False)
            else:
                # Retain Form Ref, Date, and set status to Passed for batch entry flow
                # FIX APPLIED HERE: Changed keyword from 'primary_data' to 'post_save_data'
                self._clear_form(post_save_data=primary_data, is_manual_clear=False)
    ...
    def _restore_record(self):
        # ... (Restore logic remains unchanged) ...
    ...
    def _load_all_endorsements(self):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                # UPDATED: Added remarks to the selection
                query = text(
                    f"SELECT system_ref_no, form_ref_no, date_endorsed, product_code, lot_number, quantity_kg, location, status, remarks FROM fg_endorsements_primary WHERE is_deleted IS NOT TRUE {filter_clause} ORDER BY id DESC LIMIT :limit OFFSET :offset")
                res = conn.execute(query, params).mappings().all()

            # UPDATED: Added Remarks to headers
    ...
    def _check_inventory_status(self):
        # ... (Inventory check logic remains unchanged) ...
    ...
    def _on_tab_changed(self, index):
        ...
        if tab_text == "All Records":
            ...
            # Ensure full load only happens on first entry or when manual clear is needed
    ...
    def _clear_form(self, post_save_data=None, is_manual_clear=True):
        """
        Clears the form. If post_save_data is provided, it retains Form Ref No, Date, and Status=Passed.
        If post_save_data is None, it performs a full reset (used for 'New' or 'Cancel Update').
        """
        is_cancelling = self.current_editing_ref is not None and is_manual_clear
        self.current_editing_ref, self.preview_data = None, None
        self.cancel_update_btn.hide()

        # Reset save button appearance to 'Save New'
        self.save_btn.setText("Save Endorsement")
        self.save_btn.setIcon(fa.icon('fa5s.save', color=COLOR_PRIMARY))
        self.save_btn.setObjectName("save_btn")

        if post_save_data:
            # Retention logic (after successful save)
            self.form_ref_no_edit.setText(post_save_data['form_ref_no'])

            # FIX: Use QDate constructor with year, month, day components
            py_date = post_save_data['date_endorsed']
            self.date_endorsed_edit.setDate(QDate(py_date.year, py_date.month, py_date.day))

            self.status_combo.setCurrentText("Passed")
            ...
            self.show_notification("Form ready for next entry (retained Form Ref and Date).", 'info', 3000)

        else:
            # Full clear logic (used by 'New' button, 'Cancel Update', or initial load)
            for w in [self.sys_ref_no_edit, self.form_ref_no_edit, self.lot_number_edit]: w.clear()
            self.quantity_edit.setText("0.00");
            self.weight_per_lot_edit.setText("0.00")

            # Reset combos
            if not self.lock_location_check.isChecked():
                self.location_combo.setCurrentIndex(0 if self.location_combo.count() > 0 else -1)
            for c in [self.product_code_combo, self.endorsed_by_combo, self.remarks_combo]:
                c.setCurrentIndex(-1);
                c.clearEditText()

            # Reset specific combos (Status defaults to Passed)
            self.category_combo.setCurrentIndex(0)
            self.bag_no_combo.setCurrentIndex(0 if self.bag_no_combo.count() > 0 else -1)
            self.status_combo.setCurrentText("Passed")
    ...
    def _ask_excess_handling_method(self):
        # ... (Excess handling logic remains unchanged) ...
    ...
    def _validate_and_calculate_lots(self):
        # ... (Validation and calculation logic remains unchanged) ...
        try:
            total_qty = Decimal(self.quantity_edit.text() or "0")
            weight_per_lot = Decimal(self.weight_per_lot_edit.text() or "0")
    ...
    def _perform_lot_calculation(self, total_qty, weight_per_lot, lot_input, is_range, excess_handling_method):
        # ... (Lot calculation logic remains unchanged) ...
    ...
    def _populate_preview_widgets(self, data):
        # ... (Preview population logic remains unchanged) ...
    ...
    def _populate_view_details_tab(self, sys_ref_no):
        # ... (Detail population logic remains unchanged) ...
        try:
            with self.engine.connect() as conn:
    ...
    def _add_view_detail_row(self, layout, key, value):
        ...
        if isinstance(value, datetime):
            ...
            display_text = f"{value:,.2f}"  # Added comma formatting here too
    ...
    def _populate_view_table(self, table_widget: QTableWidget, data: list, headers: list):
        # ... (View table population logic remains unchanged, uses formatting similar to below) ...
    ...
    def _load_record_for_update(self):
        ...
        try:
            ...
            # Use full clear before loading update data
            self._clear_form(post_save_data=None, is_manual_clear=False)
            ...
            self.save_btn.setIcon(fa.icon('fa5s.save', color=COLOR_PRIMARY))
            self.save_btn.setObjectName("update_btn")
    ...
    def _load_dropdown_data(self, combo: QComboBox, query_str: str, make_upper: bool = False):
        # ... (Dropdown load logic remains unchanged) ...
        try:
            with self.engine.connect() as conn:
    ...
    def _manage_list(self, table, column, title, callback):
        # ... (Manage list logic remains unchanged) ...
    ...
    def _generate_system_ref_no(self):
        # ... (Ref number generation logic remains unchanged) ...
    ...
    def _parse_lot_range(self, lot_input, num_lots_needed):
        # ... (Lot range parsing logic remains unchanged) ...
        try:
        ...
        for i, row_data in enumerate(data):
            for j, key in enumerate(keys):
                ...
                if key == 'quantity_kg' and isinstance(val, (Decimal, float)):
                    # Requirement 2: Format quantity with commas
                    item_text = f"{val:,.2f}"
    ...
    def _populate_preview_table(self, table_widget: QTableWidget, data: list, headers: list):
        # ... (Preview table population, modified to use comma formatting for consistency) ...
        table_widget.setRowCount(0);
        table_widget.setColumnCount(len(headers));
        table_widget.setHorizontalHeaderLabels(headers)
        if not data: return
