...
class FloatLineEdit(QLineEdit):
    """ A QLineEdit for float values, formatted to 2 decimal places, supporting comma separation. """
    ...
    def __init__(self, parent=None):
        ...
        self._format_text()  # Initialize to 0.00, formatted

    def _clean_text(self, text):
        # Remove commas for internal calculation/validation
        return text.replace(',', '')

    def _format_text(self):
        try:
            # Use cleaned text to get float value
            value = float(self._clean_text(self.text()) or 0.0)
            # Format the display text with comma for thousands separator
            self.setText(f"{value:,.2f}")
        except ValueError:
            self.setText("0.00")

    def value(self) -> float:
        # Return raw float after cleaning the display text
        return float(self._clean_text(self.text()) or 0.0)
...
class StandardItemEntryDialog(QDialog):
    def __init__(self, db_engine, units, prod_codes, item_data=None, parent=None):
        ...
        self.weight_pack_edit = FloatLineEdit();  # UPDATED TO USE FloatLineEdit
        self.unit_price_edit = FloatLineEdit();  # UPDATED TO USE FloatLineEdit
...
class TerumoItemEntryDialog(QDialog):
    def __init__(self, db_engine, units, prod_codes, item_data=None, parent=None):
        ...
        self.no_packing_edit = QLineEdit("0");
        self.no_packing_edit.setValidator(QIntValidator(0, 9999))
        self.weight_pack_edit = FloatLineEdit();
        self.unit_price_edit = FloatLineEdit();
    ...
    def get_item_data(self):
        ...
        return {"quantity": self.quantity_edit.text(), "unit": self.unit_edit.currentText().upper(),
                ...
                "description_1": descriptions.get("ter1", ""),
                "description_2": descriptions.get("ter2", ""),
                "attachments": self.attachments_edit.toPlainText()}
...
class ProductDeliveryPage(QWidget):
    ...
    def init_ui(self):
        ...
        header_label = QLabel("Product Delivery")
        ...
        self.tab_widget.addTab(breakdown_records_tab, fa.icon('fa5s.clipboard-list', color=ICON_COLOR),
                               "Breakdown Records")
    ...
    def _setup_breakdown_records_tab(self, tab):
        ...
        self.breakdown_records_table.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
    ...
    def _setup_lot_breakdown_tab(self, tab):
        ...
        params_group = QGroupBox("2. Define New Breakdown")
        ...
        clear_btn = QPushButton(fa.icon('fa5s.eraser', color=ICON_COLOR), "Clear All")
    ...
    def _populate_item_row(self, row, item_data):
        ...
        for col, header in enumerate(headers):
            ...
            if header in ["quantity", "weight_per_pack", "unit_price"]:
                try:
    ...
    def _get_item_data_from_row(self, row):
        ...
        for i, h in enumerate(headers):
            text = self.items_table.item(row, i).text() if self.items_table.item(row, i) else ""
            if h in ["quantity", "weight_per_pack", "unit_price"]:
                data[h] = text.replace(',', '')
            else:
                data[h] = text
        return data
    ...
    def _load_record(self, dr_no, set_current=False):
        try:
            ...
            for item_data in items:
                row = self.items_table.rowCount();
                self.items_table.insertRow(row);
                item_data_str = {k: str(v) for k, v in item_data.items()}
                self._populate_item_row(row, item_data_str)
            self._update_item_count()
            if set_current:
    ...
    def _clear_breakdown_inputs(self):
    ...
    def _recalculate_num_lots(self):
        try:
            target_qty_str = self.breakdown_dr_qty_display.text().replace(',', '')
            weight_per_lot_str = self.breakdown_weight_per_lot_edit.text().replace(',', '')
            target_qty = Decimal(target_qty_str)
            weight_per_lot = Decimal(weight_per_lot_str)
    ...
    def _validate_and_calculate_breakdown(self):
        try:
            target_qty = Decimal(self.breakdown_dr_qty_display.text().replace(',', ''))
            weight_per_lot = Decimal(self.breakdown_weight_per_lot_edit.text().replace(',', ''))
    ...
    def _load_breakdown_records_data(self):
        search_text = f"%{self.breakdown_search_edit.text()}%"
        try:
            with self.engine.connect() as conn:
                like_operator = "LIKE" if self.engine.dialect.name == 'sqlite' else "ILIKE"
                sql = text(f"""
            ...
            for row, record in enumerate(results):
                table.setItem(row, 0, QTableWidgetItem(record['dr_no']))
