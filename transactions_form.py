    ...
    def _load_transactions(self):
        ...
        try:
            with self.engine.connect() as conn:
                ...
                if search_term:
                    # NOTE: Using ILIKE for case-insensitive search (PostgreSQL).
                    # For other databases, use UPPER(column) LIKE :search and pass an uppercase search term.
                ...
                # Combine query parts
                if where_clauses:
                    query_string = f"{base_query} WHERE {' AND '.join(where_clauses)} ORDER BY id DESC"
                else:
                    query_string = f"{base_query} ORDER BY id DESC"
