...
DESTRUCTIVE_COLOR = "#dc3545"  # Red theme color for this page
HEADER_AND_ICON_COLOR = "#3a506b"
TABLE_HEADER_TEXT_COLOR = "#4f4f4f"


# A helper class for consistency
...
class FailedBeginningBalancePage(QWidget):
    ...
    def _get_styles(self) -> str:
        """Returns the stylesheet for this page, themed for 'Failed' items."""
        return f"""
            QWidget {{
                background-color: {BACKGROUND_CONTENT_COLOR}; color: {LIGHT_TEXT_COLOR}; font-family: 'Segoe UI', Arial, sans-serif;
            }}
            QGroupBox {{
                border: 1px solid #e0e5eb; border-radius: 8px; margin-top: 12px; background-color: {INPUT_BACKGROUND_COLOR};
            }}
            QGroupBox::title {{
                subcontrol-origin: margin; subcontrol-position: top left; padding: 2px 10px;
                background-color: {GROUP_BOX_HEADER_COLOR}; border: 1px solid #e0e5eb; border-bottom: 1px solid {INPUT_BACKGROUND_COLOR};
            ...
            QLineEdit, QDateEdit, QComboBox, QDoubleSpinBox {{
                border: 1px solid #d1d9e6; padding: 8px; border-radius: 5px; background-color: {INPUT_BACKGROUND_COLOR};
            }}
            QLineEdit:focus, QDateEdit:focus, QComboBox:focus, QDoubleSpinBox:focus {{
                border: 1px solid {PRIMARY_ACCENT_COLOR};
            }}
            QPushButton {{
                border: 1px solid #d1d9e6; padding: 8px 15px; border-radius: 6px; font-weight: bold; background-color: {INPUT_BACKGROUND_COLOR};
            }}
            QPushButton:hover {{ background-color: #f0f3f8; }}
            QPushButton#PrimaryButton {{
                color: {DESTRUCTIVE_COLOR}; border: 1px solid {DESTRUCTIVE_COLOR};
            }}
            QPushButton#PrimaryButton:hover {{ background-color: #fbe6e8; }}
            QPushButton#DeleteButton {{
                color: {DESTRUCTIVE_COLOR}; border: 1px solid {DESTRUCTIVE_COLOR};
            }}
            QPushButton#DeleteButton:hover {{ background-color: #fbe6e8; }}
            QTableWidget {{
                border: 1px solid #e0e5eb; background-color: {INPUT_BACKGROUND_COLOR}; selection-behavior: SelectRows; gridline-color: #f0f3f8;
            }}
            QTableWidget::item:hover {{ background-color: transparent; }}
            QTableWidget::item:selected {{
                background-color: {TABLE_SELECTION_COLOR}; color: white;
            }}
            QHeaderView::section {{
                background-color: #f4f7fc; padding: 5px; border: none; font-weight: bold; color: {TABLE_HEADER_TEXT_COLOR};
            }}
        """
    ...
    def _setup_ui(self):
        ...
        icon_label = QLabel()
        icon_label.setPixmap(fa.icon('fa5s.edit', color=DESTRUCTIVE_COLOR).pixmap(QSize(28, 28)))
        header_layout.addWidget(icon_label)
        header_layout.addWidget(QLabel(f"<h1>Beginning Balance Editor - <font color='{DESTRUCTIVE_COLOR}'>Failed</font></h1>"))
        ...
        instruction_text = (
            "Manage the foundational inventory for FAILED items. To edit, select an item from the table. "
            "To create a new one, click 'New', fill in the details, and save."
        )
        ...
        self.fg_type_combo = QComboBox(); self.fg_type_combo.addItems(["MB", "DC"])
        ...
        self.qty_spinbox = QDoubleSpinBox(); self.qty_spinbox.setRange(0, 999999.99); self.qty_spinbox.setDecimals(2)
        ...
        self.save_button = QPushButton("Save", icon=fa.icon('fa5s.save', color=DESTRUCTIVE_COLOR))
    ...
    def _load_all_records(self):
        ...
        try:
            ...
            # <<< FIX: Changed table from beginv_sheet1 to beg_invfailed1 >>>
            query_str = "SELECT id, product_code, lot_number, qty, location, fg_type, production_date FROM beg_invfailed1 WHERE 1=1"
            params = {}
            if prod_filter:
                query_str += " AND product_code ILIKE :prod"; params['prod'] = f"%{prod_filter}%"
            if lot_filter:
                query_str += " AND lot_number ILIKE :lot"; params['lot'] = f"%{lot_filter}%"
    ...
    def _populate_form_from_selection(self):
        ...
        if date_str: self.prod_date_edit.setDate(QDate.fromString(date_str, "yyyy-MM-dd"))
        try:
            with self.engine.connect() as conn:
                # <<< FIX: Changed table from beginv_sheet1 to beg_invfailed1 >>>
                result = conn.execute(text("SELECT remarks FROM beg_invfailed1 WHERE id = :id"),
        ...
        try:
            with self.engine.connect() as conn, conn.begin():
                if self.selected_record_id:
                    ...
                    # <<< FIX: Changed table from beginv_sheet1 to beg_invfailed1 >>>
                    query = text("UPDATE beg_invfailed1 SET fg_type=:fg_type, production_date=:production_date, product_code=:product_code, lot_number=:lot_number, qty=:qty, location=:location, remarks=:remarks WHERE id=:id")
                    conn.execute(query, data)
                    self.log_audit_trail("UPDATE_BEGINV_FAILED", f"Updated failed record ID {self.selected_record_id}: Lot {lot_number}")
                else:
                    # <<< FIX: Changed table from beginv_sheet1 to beg_invfailed1 >>>
                    query = text("INSERT INTO beg_invfailed1 (fg_type, production_date, product_code, lot_number, qty, location, remarks) VALUES (:fg_type, :production_date, :product_code, :lot_number, :qty, :location, :remarks)")
                    conn.execute(query, data)
                    self.log_audit_trail("INSERT_BEGINV_FAILED", f"Created new failed record: Lot {lot_number}")
    ...
    def _delete_record(self):
        ...
        reply = QMessageBox.question(self, "Confirm Delete", f"Are you sure you want to permanently delete the record for lot '{lot_number}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            try:
                with self.engine.connect() as conn, conn.begin():
                    # <<< FIX: Changed table from beginv_sheet1 to beg_invfailed1 >>>
                    conn.execute(text("DELETE FROM beg_invfailed1 WHERE id=:id"), {"id": self.selected_record_id})
                self.log_audit_trail("DELETE_BEGINV_FAILED", f"Deleted failed record ID {self.selected_record_id}: Lot {lot_number}")
